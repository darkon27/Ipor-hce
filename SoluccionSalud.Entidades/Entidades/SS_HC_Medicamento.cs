//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

using System;
using System.Collections.Generic;

namespace SoluccionSalud.Entidades.Entidades
{
    public partial class SS_HC_Medicamento
    {
        public SS_HC_Medicamento()
        {
            this.SS_HC_Indicaciones = new HashSet<SS_HC_Indicaciones>();
        }
    
        public string UnidadReplicacion { get; set; }
        public long IdEpisodioAtencion { get; set; }
        public int IdPaciente { get; set; }
        public int EpisodioClinico { get; set; }
        public int Secuencia { get; set; }
        public Nullable<int> TipoMedicamento { get; set; }
        public Nullable<int> IdUnidadMedida { get; set; }
        public string Linea { get; set; }
        public string Familia { get; set; }
        public string SubFamilia { get; set; }
        public string TipoComponente { get; set; }
        public string CodigoComponente { get; set; }
        public Nullable<int> IdVia { get; set; }
        public Nullable<decimal> Dosis { get; set; }
        public Nullable<decimal> DiasTratamiento { get; set; }
        public Nullable<decimal> Frecuencia { get; set; }
        public Nullable<decimal> Cantidad { get; set; }
        public Nullable<int> IndicadorEPS { get; set; }
        public Nullable<int> TipoReceta { get; set; }
        public Nullable<int> Forma { get; set; }
        public string Comentario { get; set; }
        public Nullable<int> IndicadorAuditoria { get; set; }
        public string UsuarioAuditoria { get; set; }
        public Nullable<int> TipoComida { get; set; }
        public Nullable<int> Estado { get; set; }
        public string UsuarioCreacion { get; set; }
        public Nullable<System.DateTime> FechaCreacion { get; set; }
        public string UsuarioModificacion { get; set; }
        public Nullable<System.DateTime> FechaModificacion { get; set; }
        public string Accion { get; set; }
        public string Version { get; set; }
    
        public virtual ICollection<SS_HC_Indicaciones> SS_HC_Indicaciones { get; set; }
        public virtual WH_ClaseSubFamilia WH_ClaseSubFamilia { get; set; }
    }
    
}
