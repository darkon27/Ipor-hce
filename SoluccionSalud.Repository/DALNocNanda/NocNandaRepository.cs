...
namespace SoluccionSalud.Repository.DALNocNanda
{
    public class NocNandaRepository : AuditGenerico<SS_HC_NandaNoc, SS_HC_NandaNoc> 
    {
        public List<SS_HC_NandaNoc> listarNocNanda(SS_HC_NandaNoc objSC, int inicio, int final)
        {
            /* dynamic DataKey = null;
             SS_HC_AuditRoyal objAudit = new SS_HC_AuditRoyal();*/
            List<SS_HC_NandaNoc> objLista = new List<SS_HC_NandaNoc>();

            using (var context = new WEB_ERPSALUDEntities())
            {
                objLista = context.SP_SS_HC_NocNanda_LISTAR(
        ...
        public int setMantenimientoNanNoc(SS_HC_NandaNoc objSC)
        {
            /*  SS_HC_AuditRoyal objAudit = new SS_HC_AuditRoyal();*/
            /* dynamic DataKey;*/
            ...
            using (var context = new WEB_ERPSALUDEntities())
            {
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        SS_HC_NandaNoc objAnterior = new SS_HC_NandaNoc();
                        if ((objSC.Accion.Substring(0, 1) != "I") || (objSC.Accion.Substring(0, 1) != "N"))
                        {
                            objAnterior = (from t in context.SS_HC_NandaNoc
                                           where t.IdNanNoc == objSC.IdNanNoc
                                           orderby t.IdNanNoc descending
                                           select t).SingleOrDefault();
                        }
                        iReturnValue = context.SP_SS_HC_NOC_NANDA(
                            objSC.IdNanNoc,
                            objSC.IdNanda,
                            objSC.IdNoc,
                            objSC.Descripcion,
