...
{
    public class EvaluacionDolorEvaNiniosFERepository
    {
        ...
        public List<SS_HC_Evaluacion_DolorEvaNinios_FE> listarEntidad(SS_HC_Evaluacion_DolorEvaNinios_FE objSC)
        {
            List<SS_HC_Evaluacion_DolorEvaNinios_FE> objLista = new List<SS_HC_Evaluacion_DolorEvaNinios_FE>();

            using (var context = new ModelFormularios())
            {
                List<SS_HC_Evaluacion_DolorEvaNinios_FE> objConsulta = (from t in context.SS_HC_Evaluacion_DolorEvaNinios_FE
                                                                         where
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    objAudit = Control.AddAudita(new SS_HC_Evaluacion_DolorEvaNinios_FE(), new SS_HC_Evaluacion_DolorEvaNinios_FE(), DataKey, objAudit.Type);
                    String xlmIn = XMLString(objLista, "SS_HC_Evaluacion_DolorEvaNinios_FE");
                    objAudit.TableName = "SS_HC_Evaluacion_DolorEvaNinios_FE";
        ...
        public int setMantenimiento(SS_HC_Evaluacion_DolorEvaNinios_FE objSC)
        {
            ...
            using (var context = new ModelFormularios())
            {
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            int objCount = 0;
                            //  SS_HC_Evaluacion_DolorEvaNinios_FE objAnterior = null;
                            if (objSC.Accion == null) objSC.Accion = "X";
                            if ((objSC.Accion.Substring(0, 1) != "I") && (objSC.Accion.Substring(0, 1) != "N"))
                            {
                                objCount = context.SS_HC_Evaluacion_DolorEvaNinios_FE
                                                .Where(t =>
                                                    t.UnidadReplicacion == objSC.UnidadReplicacion
                                                    && t.IdPaciente == objSC.IdPaciente
                                                    && t.EpisodioClinico == objSC.EpisodioClinico
                            ...
                            if (objCount > 0)
                            {
                                //objSC.IdDolorEvaNinios = 1; //PK
                                if (objSC.Accion == "UPDATE")
                                {
                                    objSC.Version = "CCEPF432";
        ...
        public virtual String XMLString(List<SS_HC_Evaluacion_DolorEvaNinios_FE> obPadre, String TablaID)
        {
