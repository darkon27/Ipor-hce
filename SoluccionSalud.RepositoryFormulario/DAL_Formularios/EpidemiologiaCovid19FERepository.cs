...
{
    public partial class EpidemiologiaCovid19FERepository
    {
        ...
        public List<SS_HC_Epidemiologia_FE> listarEntidad(SS_HC_Epidemiologia_FE objSC)
        {
            List<SS_HC_Epidemiologia_FE> objLista = new List<SS_HC_Epidemiologia_FE>();
            using (var context = new ModelFormularios())
            {
                List<SS_HC_Epidemiologia_FE> objConsultas = (from t in context.SS_HC_Epidemiologia_FE
                                                             where
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    objAudit = Control.AddAudita(new SS_HC_Epidemiologia_FE(), new SS_HC_Epidemiologia_FE(), DataKey, objAudit.Type);
                    String xlmIn = XMLString(objLista, "SS_HC_Epidemiologia_FE");
                    objAudit.TableName = "SS_HC_Epidemiologia_FE";
        ...
        public int setMantenimiento(SS_HC_Epidemiologia_FE objSC, List<SS_HC_EpidemiologiaDetalleLugar_FE> ObjDetalle, List<SS_HC_EpidemiologiaDetalleCasos_FE> ObjCasosDetalle2
           )
        {
                ...
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (objSC != null)
                        {
                            ...
                            SS_HC_Epidemiologia_FE objAnterior = null;
                            if (objSC.Accion == null) objSC.Accion = "X";
                            if ((objSC.Accion.Substring(0, 1) != "I") && (objSC.Accion.Substring(0, 1) != "N"))
                            {
                                objAnterior = (from t in context.SS_HC_Epidemiologia_FE
                                               where
                                               t.UnidadReplicacion == objSC.UnidadReplicacion
                                               && t.IdPaciente == objSC.IdPaciente
                                               && t.EpisodioClinico == objSC.EpisodioClinico
                        ...
                        if (ObjDetalle != null)
                        {
                            ...
                            var secuenciaMax = context.SS_HC_EpidemiologiaDetalleLugar_FE
                                    .Where(t =>
                                            t.IdPaciente == objSC.IdPaciente
                                            && t.UnidadReplicacion == objSC.UnidadReplicacion
                                            && t.EpisodioClinico == objSC.EpisodioClinico
                            ...
                            foreach (SS_HC_EpidemiologiaDetalleLugar_FE objDetalle in ObjDetalle)
                            {
                                ...
                                else if (objDetalle.Accion == "DELETEDETALLE")  // Eliminar Detalle
                                {
                                    ...
                                    using (var contextEnty = new WEB_ERPSALUDEntities())
                                    {
                                        ...
                                        string tempType = objAudit.Type;
                                        objAudit = Control.AddAudita(new SS_HC_EpidemiologiaDetalleLugar_FE(), new SS_HC_EpidemiologiaDetalleLugar_FE(), DataKey, objAudit.Type);
                                        objAudit.TableName = "SS_HC_EpidemiologiaDetalleLugar_FE";
                                        objAudit.Type = "D";
                        ...
                        /*********GUARDAR DETALLE 2 Casos ***************/

                        if (ObjCasosDetalle2 != null)
                        {
                            /**obtener la última secuencia*/
                            var secuenciaMaxDos = context.SS_HC_EpidemiologiaDetalleCasos_FE
                                    .Where(t =>
                                            t.IdPaciente == objSC.IdPaciente
                                            && t.UnidadReplicacion == objSC.UnidadReplicacion
                                            && t.EpisodioClinico == objSC.EpisodioClinico
                            ...
                            foreach (SS_HC_EpidemiologiaDetalleCasos_FE objDetalle2Cas in ObjCasosDetalle2)
                            {

                                //int secuenciaAuxDos = objDetalleDos.Secuencia;
                                if (objDetalle2Cas.Accion == "DETALLE") // Insertar Detalle
                                {
                                    contadorDetDos++;
                                    objDetalle2Cas.Secuencia = secuenciaMaxDos + contadorDetDos;
                                    context.Entry(objDetalle2Cas).State = EntityState.Added;
                                }
                                else if (objDetalle2Cas.Accion == "UPDATEDETALLE") // Actualizar Detalle
                                {
                                    context.Entry(objDetalle2Cas).State = EntityState.Modified;
                                    //mapSecuencia.Add("" + secuenciaAuxDos, objDetalleDos.Secuencia);

                                }
                                else if (objDetalle2Cas.Accion == "DELETEDETALLE")  // Eliminar Detalle
                                {
                                    context.Entry(objDetalle2Cas).State = EntityState.Deleted;
                                    ...
                                    using (var contextEnty = new WEB_ERPSALUDEntities())
                                    {
                                        ...
                                        string tempType = objAudit.Type;
                                        objAudit = Control.AddAudita(new SS_HC_EpidemiologiaDetalleCasos_FE(), new SS_HC_EpidemiologiaDetalleCasos_FE(), DataKey, objAudit.Type);
                                        objAudit.TableName = "SS_HC_EpidemiologiaDetalleCasos_FE ";
                                        objAudit.Type = "D";
                                        objAudit.Accion = "DELETEDETALLE";
                                        contextEnty.Entry(objAudit).State = EntityState.Added;
                                        contextEnty.SaveChanges();
        ...
        public virtual String XMLString(List<SS_HC_Epidemiologia_FE> obPadre, String TablaID)
        {
