...
namespace SoluccionSalud.RepositoryFormulario.DAL_Formularios
{
    public class AlergiaRepository
    {
        ...
        public int setMantenimiento(SS_HC_Alergia_FE ObjTraces, List<SS_HC_Alergia_FE> Cabecera, List<SS_HC_Alergia_FE> Detalle)
        {     
            SS_HC_AlergiaDetalle_FE objAnamSintoma;
            SS_HC_AlergiaDetalle_FE InforDetalleObj = new SS_HC_AlergiaDetalle_FE();
            InforDetalleObj.SS_HC_Alergia_FE = null;
            ...
            using (var context = new ModelFormularios())
            {
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    ...
                    {
                        // C A B E C E R A
                        foreach (SS_HC_Alergia_FE ObjTrace in Cabecera)
                        {
                            var InformacionObj = (from t in context.SS_HC_Alergia_FE
                            ...
                            if (InformacionObj == null) InformacionObj = new SS_HC_Alergia_FE();

                            iReturnValue = context.USP_Alergia_FE(ObjTrace.UnidadReplicacion, ObjTrace.IdEpisodioAtencion, 
                                          ObjTrace.IdPaciente,ObjTrace.EpisodioClinico,
                                          ObjTrace.TieneHistoria, 
                                          ...
                                          ObjTrace.ObservacionesDet, ObjTrace.TipoRegistroDet, ObjTrace.DescripcionManualDet).SingleOrDefault();

                            valorRetorno = Convert.ToInt32(iReturnValue.ToString().Trim());
                            // D E T A L L E
                            foreach (SS_HC_Alergia_FE ObjTraceDell in Detalle)
                            {
                                if (ObjTraceDell.Accion == "DELETEDETALLE")
                                {
                                    ...
                                    using (var contextEnty = new WEB_ERPSALUDEntities())
                                    {
                                        ...
                                        objAudit = Control.AddAudita(new SS_HC_Alergia_FE(), new SS_HC_Alergia_FE(), DataKey, objAudit.Type);
                                        objAudit.TableName = "SS_HC_Alergia_FE";
                                        objAudit.Type = "D";
                                ...
                                var virtualObj = (from t in context.SS_HC_AlergiaDetalle_FE
                                ...
                                // Para la Audotoria
                                objAnamSintoma = new SS_HC_AlergiaDetalle_FE();
                                ...
                                context.USP_Alergia_FE(ObjTraceDell.UnidadReplicacion, ObjTraceDell.IdEpisodioAtencion, 
                                      ObjTraceDell.IdPaciente, ObjTraceDell.EpisodioClinico,
                                      ObjTraceDell.TieneHistoria,
                                      ...
                                      ObjTraceDell.IdMedicamento,
                                      ObjTraceDell.LineaDet, ObjTraceDell.FamiliaDet, ObjTraceDell.SubFamiliaDet,
                                      ObjTraceDell.IdTipoAlergiaDet, ObjTraceDell.Accion, ObjTraceDell.DesdeCuandoDet,
                                      ObjTraceDell.EstudioAlegistaDet, ObjTraceDell.ObservacionesDet, ObjTraceDell.TipoRegistroDet, ObjTraceDell.DescripcionManualDet).SingleOrDefault();                                
        ...
        public int setMantenimiento(SS_HC_Alergia_FE ObjTrace)
        {
            System.Nullable<int> iReturnValue;
            int valorRetorno = 0;
            using (var context = new ModelFormularios())
            {
                try
                {
                    iReturnValue = context.USP_Alergia_FE(ObjTrace.UnidadReplicacion, ObjTrace.IdEpisodioAtencion, ObjTrace.IdPaciente, ObjTrace.EpisodioClinico,
                      ObjTrace.TieneHistoria, ObjTrace.IdFormaInicio, ObjTrace.IdCursoEnfermedad,                      
                      ObjTrace.Estado, ObjTrace.UsuarioCreacion,
                      ObjTrace.FechaCreacion, ObjTrace.UsuarioModificacion, ObjTrace.FechaModificacion,
                      ObjTrace.IdMedicamento, ObjTrace.LineaDet, ObjTrace.FamiliaDet, ObjTrace.SubFamiliaDet,
                      ObjTrace.IdTipoAlergiaDet, ObjTrace.Accion, ObjTrace.DesdeCuandoDet, ObjTrace.EstudioAlegistaDet, ObjTrace.ObservacionesDet,"","").SingleOrDefault();
                    valorRetorno = Convert.ToInt32(iReturnValue.ToString().Trim());
        ...
        public List<SS_HC_Alergia_FE> Alergia_Listar(SS_HC_Alergia_FE ObjTrace)
        {
            try
            {               
                List<SS_HC_Alergia_FE> objLista = new List<SS_HC_Alergia_FE>();
                using (var context = new ModelFormularios())
                {
                    objLista = context.USP_Alergia_FE_Listar(ObjTrace.UnidadReplicacion, ObjTrace.IdEpisodioAtencion, ObjTrace.IdPaciente, ObjTrace.EpisodioClinico,
                         ObjTrace.TieneHistoria,
                         ObjTrace.IdFormaInicio, ObjTrace.IdCursoEnfermedad,                         
                         ObjTrace.Estado, ObjTrace.UsuarioCreacion,
                         ObjTrace.FechaCreacion, ObjTrace.UsuarioModificacion, ObjTrace.FechaModificacion, 
                         "","",  ObjTrace.IdMedicamento,  ObjTrace.Accion).ToList();
                    ...
                    using (var contextEnty = new WEB_ERPSALUDEntities())
                    {
                        ...
                        string tempType = objAudit.Type;
                        objAudit = Control.AddAudita(new SS_HC_Alergia_FE(), new SS_HC_Alergia_FE(), DataKey, objAudit.Type);
                        String xlmIn = XMLString(objLista, "SS_HC_Alergia_FE");
                        objAudit.TableName = "SS_HC_Alergia_FE";
        ...
        public virtual String XMLString(List<SS_HC_Alergia_FE> obPadre, String TablaID)
        {
