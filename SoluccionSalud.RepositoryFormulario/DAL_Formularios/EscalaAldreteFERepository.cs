...
{
    public class EscalaAldreteFERepository
    {
        ...
        public List<SS_HC_EscalaAldrete_FE> listarEntidad(SS_HC_EscalaAldrete_FE objSC)
        {


            List<SS_HC_EscalaAldrete_FE> objLista = new List<SS_HC_EscalaAldrete_FE>();
            using (var context = new ModelFormularios())
            {

                List<SS_HC_EscalaAldrete_FE> objConsultas = (from t in context.SS_HC_EscalaAldrete_FE
                                                             where
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    objAudit = Control.AddAudita(new SS_HC_EscalaAldrete_FE(), new SS_HC_EscalaAldrete_FE(), DataKey, objAudit.Type);
                    String xlmIn = XMLString(objLista, "SS_HC_EscalaAldrete_FE");
                    objAudit.TableName = "SS_HC_EscalaAldrete_FE";
        ...
        public int setMantenimiento(SS_HC_EscalaAldrete_FE objSC)
        {
                ...
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        if (objSC != null)
                        {
                            ...
                            {
                                objCount = context.SS_HC_EscalaAldrete_FE
                                                .Where(t =>
                                                    t.UnidadReplicacion == objSC.UnidadReplicacion
                                                    && t.IdPaciente == objSC.IdPaciente
                                                    && t.EpisodioClinico == objSC.EpisodioClinico
                            ...
                            else
                            {
                                if (objSC.Accion == "NUEVO")
                                {
                                    ...
                                    var secuenciaMax = context.SS_HC_EscalaAldrete_FE
                                                     .DefaultIfEmpty().Max(t => t == null ? 0 : t.IdEscalaAldrete);
                                    objSC.IdEscalaAldrete = secuenciaMax + 1;
                                    objSC.Version = "CCEPF444";
        ...
        public virtual String XMLString(List<SS_HC_EscalaAldrete_FE> obPadre, String TablaID)
        {
