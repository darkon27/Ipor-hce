...
{
    public partial class VigilanciaDispositivosFERepository
    {
        ...
        public List<SS_HC_VIGILANCIA_DISPOSITIVOS_FE> listarEntidad(SS_HC_VIGILANCIA_DISPOSITIVOS_FE objSC)
        {
            List<SS_HC_VIGILANCIA_DISPOSITIVOS_FE> objLista = new List<SS_HC_VIGILANCIA_DISPOSITIVOS_FE>();
            using (var context = new ModelFormularios())
            {
                List<SS_HC_VIGILANCIA_DISPOSITIVOS_FE> objConsultas = (from t in context.SS_HC_VIGILANCIA_DISPOSITIVOS_FE
                                                                where
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    objAudit = Control.AddAudita(new SS_HC_VIGILANCIA_DISPOSITIVOS_FE(), new SS_HC_VIGILANCIA_DISPOSITIVOS_FE(), DataKey, objAudit.Type);
                    String xlmIn = XMLString(objLista, "SS_HC_VIGILANCIA_DISPOSITIVOS_FE");
                    objAudit.TableName = "SS_HC_VIGILANCIA_DISPOSITIVOS_FE";
        ...
        public int setMantenimiento(SS_HC_VIGILANCIA_DISPOSITIVOS_FE objSC)
        {
                ...
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        if (objSC != null)
                        {
                            ...
                            {
                                objCount = context.SS_HC_VIGILANCIA_DISPOSITIVOS_FE
                                                .Where(t =>
                                                    t.UnidadReplicacion == objSC.UnidadReplicacion
                                                    && t.IdPaciente == objSC.IdPaciente
                                                    && t.EpisodioClinico == objSC.EpisodioClinico
                            ...
                            valor = context.SS_HC_VIGILANCIA_DISPOSITIVOS_FE.Count();
                            /**TRANSACCIÓN*/
                            if (objCount > 0)
                            {
                                if (objSC.Accion == "UPDATE")
                                {
                                    objSC.Version = "CCEPF425";
                            ...
                            else
                            {
                                ...
                                {
                                    var secuenciaMax = context.SS_HC_VIGILANCIA_DISPOSITIVOS_FE
                                                     .DefaultIfEmpty().Max(t => t == null ? 0 : t.Idvigilancia_dispositivos);
                                    objSC.Idvigilancia_dispositivos = secuenciaMax + 1;
                                    objSC.Version = "CCEPF425";
        ...
        public virtual String XMLString(List<SS_HC_VIGILANCIA_DISPOSITIVOS_FE> obPadre, String TablaID)
        {
