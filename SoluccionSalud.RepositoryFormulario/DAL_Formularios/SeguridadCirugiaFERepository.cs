...
{
    public class SeguridadCirugiaFERepository
    {
        ...
        public List<SS_HC_SeguridadCirugia_FE> listarEntidad(SS_HC_SeguridadCirugia_FE objSC)
        {


            List<SS_HC_SeguridadCirugia_FE> objLista = new List<SS_HC_SeguridadCirugia_FE>();
            using (var context = new ModelFormularios())
            {

                List<SS_HC_SeguridadCirugia_FE> objConsultas = (from t in context.SS_HC_SeguridadCirugia_FE
                                                               ...
                                                               && t.TipoCirugia == objSC.TipoCirugia
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    objAudit = Control.AddAudita(new SS_HC_SeguridadCirugia_FE(), new SS_HC_SeguridadCirugia_FE(), DataKey, objAudit.Type);
                    String xlmIn = XMLString(objLista, "SS_HC_SeguridadCirugia_FE");
                    objAudit.TableName = "SS_HC_SeguridadCirugia_FE";
        ...
        public int setMantenimiento(SS_HC_SeguridadCirugia_FE objSC)
        {
                ...
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        if (objSC != null)
                        {
                            ...
                            {
                                objCount = context.SS_HC_SeguridadCirugia_FE
                                                .Where(t =>
                                                    t.UnidadReplicacion == objSC.UnidadReplicacion
                                                    && t.IdPaciente == objSC.IdPaciente
                                                    && t.EpisodioClinico == objSC.EpisodioClinico
                                                    && t.IdEpisodioAtencion == objSC.IdEpisodioAtencion
                                                    && t.IdSeguridad == objSC.IdSeguridad
                                                 )
                                               .DefaultIfEmpty()
                                               .Count();
                            }
                            ...
                            /**TRANSACCIÓN*/
                                ...
                                if (objCount > 0)
                                {
                                    if (objSC.TipoCirugia == 1)
                                    {
                                        if (objSC.Accion == "UPDATE")
                                        {
                                            objSC.Version = "CCEPF461";
                                            context.Entry(objSC).State = EntityState.Modified;
                                        } 
                                    }
                                    else if (objSC.TipoCirugia == 2)
                                    {
                                        if (objSC.Accion == "UPDATE")
                                        {
                                            objSC.Version = "CCEPF462";
                                            context.Entry(objSC).State = EntityState.Modified;
                                        }
                                    }
                                    else if (objSC.TipoCirugia == 3)
                                    {
                                        if (objSC.Accion == "UPDATE")
                                        {
                                            objSC.Version = "CCEPF463";
                                            context.Entry(objSC).State = EntityState.Modified;
                                        }
                                    }
                            ...
                            else
                            {
                                ...
                                {
                                    var secuenciaMax = context.SS_HC_SeguridadCirugia_FE
                                                     .DefaultIfEmpty().Max(t => t == null ? 0 : t.IdSeguridad);

                                    objSC.IdSeguridad = secuenciaMax + 1;
                            ...
                            using (var contextEnty = new WEB_ERPSALUDEntities())
                            {
                                ...
                                DataKey = new
                                {
                                    ...
                                    EpisodioClinico = objSC.EpisodioClinico,
                                    IdPaciente = objSC.IdPaciente,
                                    IdSeguridad = objSC.IdSeguridad,
                                };
                                string tempType = objAudit.Type;
                                objAudit = Control.AddAudita(new SS_HC_SeguridadCirugia_FE(), new SS_HC_SeguridadCirugia_FE(), DataKey, objAudit.Type);
                                objAudit.TableName = "SS_HC_SeguridadCirugia_Pausa_FE ";
        ...
        public virtual String XMLString(List<SS_HC_SeguridadCirugia_FE> obPadre, String TablaID)
        {
