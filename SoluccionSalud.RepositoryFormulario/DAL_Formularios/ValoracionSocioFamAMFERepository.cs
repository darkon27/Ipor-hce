...
{
    public class ValoracionSocioFamAMFERepository
    {
        ...
        public List<SS_HC_ValoracionSocioFamAM_FE> listarEntidad(SS_HC_ValoracionSocioFamAM_FE objSC)
        {


            List<SS_HC_ValoracionSocioFamAM_FE> objLista = new List<SS_HC_ValoracionSocioFamAM_FE>();
            using (var context = new ModelFormularios())
            {

                List<SS_HC_ValoracionSocioFamAM_FE> objConsultas = (from t in context.SS_HC_ValoracionSocioFamAM_FE
                                                           where
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    objAudit = Control.AddAudita(new SS_HC_ValoracionSocioFamAM_FE(), new SS_HC_ValoracionSocioFamAM_FE(), DataKey, objAudit.Type);
                    String xlmIn = XMLString(objLista, "SS_HC_ValoracionSocioFamAM_FE");
                    objAudit.TableName = "SS_HC_ValoracionSocioFamAM_FE";
        ...
        public int setMantenimiento(SS_HC_ValoracionSocioFamAM_FE objSC)
        {
            ...
            using (var context = new ModelFormularios())
            {
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            int objCount = 0;
                           // SS_HC_ValoracionSocioFamAM_FE objAnterior = null;
                            if (objSC.Accion == null) objSC.Accion = "X";
                            if ((objSC.Accion.Substring(0, 1) != "I") && (objSC.Accion.Substring(0, 1) != "N"))
                            {
                               objCount = context.SS_HC_ValoracionSocioFamAM_FE
                                               .Where(t =>
                                                   t.UnidadReplicacion == objSC.UnidadReplicacion
                                                   && t.IdPaciente == objSC.IdPaciente
                                                   && t.EpisodioClinico == objSC.EpisodioClinico
        ...
        public virtual String XMLString(List<SS_HC_ValoracionSocioFamAM_FE> obPadre, String TablaID)
        {
