...
{
    public class VigilanciaCateterUrinarioFERepository
    {
        ...
        public List<SS_HC_ENFER_VIGI_CATETERURINARIO_FE> listarSS_HC_ENFER_VIGI_CATETERURINARIO_FE(SS_HC_ENFER_VIGI_CATETERURINARIO_FE objSC)
        {
            List<SS_HC_ENFER_VIGI_CATETERURINARIO_FE> objLista = new List<SS_HC_ENFER_VIGI_CATETERURINARIO_FE>();
            using (var context = new ModelFormularios())
            {
                List<SS_HC_ENFER_VIGI_CATETERURINARIO_FE> objConsultas = (from t in context.SS_HC_ENFER_VIGI_CATETERURINARIO_FE
                                                                          where
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    objAudit = Control.AddAudita(new SS_HC_ENFER_VIGI_CATETERURINARIO_FE(), new SS_HC_ENFER_VIGI_CATETERURINARIO_FE(), DataKey, objAudit.Type);
                    String xlmIn = XMLString(objLista, "SS_HC_ENFER_VIGI_CATETERURINARIO_FE");
                    objAudit.TableName = "SS_HC_ENFER_VIGI_CATETERURINARIO_FE";
        ...
        public int setMantenimiento(SS_HC_ENFER_VIGI_CATETERURINARIO_FE objSC)
        {
                ...
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        if (objSC != null)
                        {
                            ...
                            {
                                objCount = context.SS_HC_ENFER_VIGI_CATETERURINARIO_FE
                                                .Where(t =>
                                                    t.UnidadReplicacion == objSC.UnidadReplicacion
                                                    && t.IdPaciente == objSC.IdPaciente
                                                    && t.EpisodioClinico == objSC.EpisodioClinico
                            ...
                            valor = context.SS_HC_ENFER_VIGI_CATETERURINARIO_FE.Count();
                            /**TRANSACCIÓN*/
                            if (objCount > 0)
                            {
                                if (objSC.Accion == "UPDATE")
                                {
                                    objSC.Version = "CCEPF420";
                            ...
                            else
                            {
                                ...
                                {
                                    var secuenciaMax = context.SS_HC_ENFER_VIGI_CATETERURINARIO_FE
                                                     .DefaultIfEmpty().Max(t => t == null ? 0 : t.IdCateterUrinario);
                                    objSC.IdCateterUrinario = secuenciaMax + 1;
                                    objSC.Version = "CCEPF420";
        ...
        public virtual String XMLString(List<SS_HC_ENFER_VIGI_CATETERURINARIO_FE> obPadre, String TablaID)
        {
