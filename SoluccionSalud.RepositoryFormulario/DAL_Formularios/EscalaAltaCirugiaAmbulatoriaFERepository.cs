...
{
    public  class EscalaAltaCirugiaAmbulatoriaFERepository
    {
...
public List<SS_HC_EscalaAltaCirugiaAmbulatoria_FE> listarEntidad(SS_HC_EscalaAltaCirugiaAmbulatoria_FE objSC)
        {


            List<SS_HC_EscalaAltaCirugiaAmbulatoria_FE> objLista = new List<SS_HC_EscalaAltaCirugiaAmbulatoria_FE>();
            using (var context = new ModelFormularios())
            {

                List<SS_HC_EscalaAltaCirugiaAmbulatoria_FE> objConsultas = (from t in context.SS_HC_EscalaAltaCirugiaAmbulatoria_FE
                                                             where
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    objAudit = Control.AddAudita(new SS_HC_EscalaAltaCirugiaAmbulatoria_FE(), new SS_HC_EscalaAltaCirugiaAmbulatoria_FE(), DataKey, objAudit.Type);
                    String xlmIn = XMLString(objLista, "SS_HC_EscalaAltaCirugiaAmbulatoria_FE");
                    objAudit.TableName = "SS_HC_EscalaAltaCirugiaAmbulatoria_FE";
...
public int setMantenimiento(SS_HC_EscalaAltaCirugiaAmbulatoria_FE objSC)
        {
                ...
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        if (objSC != null)
                        {
                            ...
                            {
                                objCount = context.SS_HC_EscalaAltaCirugiaAmbulatoria_FE
                                                .Where(t =>
                                                    t.UnidadReplicacion == objSC.UnidadReplicacion
                                                    && t.IdPaciente == objSC.IdPaciente
                                                    && t.EpisodioClinico == objSC.EpisodioClinico
                            ...
                            valor = context.SS_HC_EscalaAltaCirugiaAmbulatoria_FE.Count();
                            /**TRANSACCIÓN*/
                            if (objCount > 0)
                            {
                            ...
                            else
                            {
                                if (objSC.Accion == "NUEVO")
                                {
                                    ...
                                    var secuenciaMax = context.SS_HC_EscalaAltaCirugiaAmbulatoria_FE
                                                     .DefaultIfEmpty().Max(t => t == null ? 0 : t.IdEscalaAltaCirugiaAmb);
                                    objSC.IdEscalaAltaCirugiaAmb = secuenciaMax + 1;
                                    objSC.Version = "CCEPF464";
        ...
        public virtual String XMLString(List<SS_HC_EscalaAltaCirugiaAmbulatoria_FE> obPadre, String TablaID)
        {
