...
{
    public class ValoracionFuncionalAMFERepository
    {
        ...
        public List<SS_HC_ValoracionFuncionalAM_FE> listarEntidad(SS_HC_ValoracionFuncionalAM_FE objSC)
        {


            List<SS_HC_ValoracionFuncionalAM_FE> objLista = new List<SS_HC_ValoracionFuncionalAM_FE>();
            using (var context = new ModelFormularios())
            {

                List<SS_HC_ValoracionFuncionalAM_FE> objConsultas = (from t in context.SS_HC_ValoracionFuncionalAM_FE
                                                                   where
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    String xlmIn = XMLString(objLista, "SS_HC_ValoracionFuncionalAM_FE");
                    objAudit.TableName = "SS_HC_ValoracionFuncionalAM_FE";
        ...
        public int setMantenimiento(SS_HC_ValoracionFuncionalAM_FE objSC)
        {
            ...
            using (var context = new ModelFormularios())
            {
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        if (objSC != null)
                        {
                            //SS_HC_ValoracionFuncionalAM_FE objAnterior = null;
                            int objCount = 0;
                            if (objSC.Accion == null) objSC.Accion = "X";
                            if ((objSC.Accion.Substring(0, 1) != "I") && (objSC.Accion.Substring(0, 1) != "N"))
                            {
                                objCount = context.SS_HC_ValoracionFuncionalAM_FE
                                               .Where(t =>
                                                   t.UnidadReplicacion == objSC.UnidadReplicacion
                                                   && t.IdPaciente == objSC.IdPaciente
                                                   && t.EpisodioClinico == objSC.EpisodioClinico
        ...
        public virtual String XMLString(List<SS_HC_ValoracionFuncionalAM_FE> obPadre, String TablaID)
        {
