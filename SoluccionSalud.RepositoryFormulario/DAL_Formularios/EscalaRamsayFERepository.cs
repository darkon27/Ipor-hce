...
{
    public class EscalaRamsayFERepository
    {
        ...
        public List<SS_HC_EscalaRamsay_FE> listarEntidad(SS_HC_EscalaRamsay_FE objSC)
        {


            List<SS_HC_EscalaRamsay_FE> objLista = new List<SS_HC_EscalaRamsay_FE>();
            using (var context = new ModelFormularios())
            {

                List<SS_HC_EscalaRamsay_FE> objConsultas = (from t in context.SS_HC_EscalaRamsay_FE
                                                            where
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    objAudit = Control.AddAudita(new SS_HC_EscalaRamsay_FE(), new SS_HC_EscalaRamsay_FE(), DataKey, objAudit.Type);
                    String xlmIn = XMLString(objLista, "SS_HC_EscalaRamsay_FE");
                    objAudit.TableName = "SS_HC_EscalaRamsay_FE";
        ...
        public int setMantenimiento(SS_HC_EscalaRamsay_FE objSC)
        {
                ...
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        if (objSC != null)
                        {
                            ...
                            {
                                objCount = context.SS_HC_EscalaRamsay_FE
                                                .Where(t =>
                                                    t.UnidadReplicacion == objSC.UnidadReplicacion
                                                    && t.IdPaciente == objSC.IdPaciente
                                                    && t.EpisodioClinico == objSC.EpisodioClinico
                            ...
                            valor = context.SS_HC_SeguridadCirugia_Pausa_FE.Count();
                            /**TRANSACCIÓN*/
                            if (objCount > 0)
                            {
                                if (objSC.Accion == "UPDATE")
                                {
                                    ...
                                    using (ModelFormularios db = new ModelFormularios())
                                    {
                                        ...
                                        var Otabla = db.SS_HC_EscalaRamsay_FE.SingleOrDefault(b => b.IdEscalaRamsay == ENTITY_GLOBAL.Instance.CapIdEscalaRamsay);



                                        objSC.Version = "CCEPF447";
                                        Otabla.FechaIngreso = objSC.FechaIngreso;
                                        Otabla.HoraIngreso = objSC.HoraIngreso;
                                        Otabla.Turno = objSC.Turno;
                                        Otabla.FlagParametro1 = objSC.FlagParametro1;
                                        Otabla.DescripcionSedacion = objSC.DescripcionSedacion;
                            ...
                            else
                            {
                                if (objSC.Accion == "NUEVO")
                                {
                                    ...
                                    var secuenciaMax = context.SS_HC_EscalaRamsay_FE
                                                     .DefaultIfEmpty().Max(t => t == null ? 0 : t.IdEscalaRamsay);
                                    objSC.IdEscalaRamsay = secuenciaMax + 1;
                                    objSC.Version = "CCEPF447";
        ...
        public virtual String XMLString(List<SS_HC_EscalaRamsay_FE> obPadre, String TablaID)
        {
