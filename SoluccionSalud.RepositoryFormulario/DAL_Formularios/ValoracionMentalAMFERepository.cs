...
{
    public class ValoracionMentalAMFERepository
    {
        ...
        public List<SS_HC_ValoracionMentalAM_FE> listarEntidad(SS_HC_ValoracionMentalAM_FE objSC)
        {


            List<SS_HC_ValoracionMentalAM_FE> objLista = new List<SS_HC_ValoracionMentalAM_FE>();
            using (var context = new ModelFormularios())
            {

                List<SS_HC_ValoracionMentalAM_FE> objConsultas = (from t in context.SS_HC_ValoracionMentalAM_FE
                                                           where
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    objAudit = Control.AddAudita(new SS_HC_ValoracionMentalAM_FE(), new SS_HC_ValoracionMentalAM_FE(), DataKey, objAudit.Type);
                    String xlmIn = XMLString(objLista, "SS_HC_ValoracionMentalAM_FE");
                    objAudit.TableName = "SS_HC_ValoracionMentalAM_FE";
        ...
        public int setMantenimiento(SS_HC_ValoracionMentalAM_FE objSC)
        {
            int valorRetorno = 0;
            ...
            using (var context = new ModelFormularios())
            {
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            int objCount = 0;
                            //SS_HC_ValoracionMentalAM_FE objAnterior = null;
                            if (objSC.Accion == null) objSC.Accion = "X";

                            if ((objSC.Accion.Substring(0, 1) != "I") && (objSC.Accion.Substring(0, 1) != "N"))
                            {
                                objCount = context.SS_HC_ValoracionMentalAM_FE
                                                .Where(t =>
                                                   t.UnidadReplicacion == objSC.UnidadReplicacion
                                                   && t.IdPaciente == objSC.IdPaciente
                                                   && t.EpisodioClinico == objSC.EpisodioClinico
                            ...
                            else
                            {
                                ...
                                {
                                    objSC.Version = "CCEPF016";                                
                                    context.Entry(objSC).State = EntityState.Added; // En caso usa?
                                }
        ...
        public virtual String XMLString(List<SS_HC_ValoracionMentalAM_FE> obPadre, String TablaID)
        {
