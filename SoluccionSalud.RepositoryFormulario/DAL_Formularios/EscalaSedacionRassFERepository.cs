...
{
    public class EscalaSedacionRassFERepository
    {
        ...
        public List<SS_HC_EscalaSedacionRass_FE> listarEntidad(SS_HC_EscalaSedacionRass_FE objSC)
        {
            List<SS_HC_EscalaSedacionRass_FE> objLista = new List<SS_HC_EscalaSedacionRass_FE>();
            using (var context = new ModelFormularios())
            {
                List<SS_HC_EscalaSedacionRass_FE> objConsultas = (from t in context.SS_HC_EscalaSedacionRass_FE
                                                             where
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    objAudit = Control.AddAudita(new SS_HC_EscalaSedacionRass_FE(), new SS_HC_EscalaSedacionRass_FE(), DataKey, objAudit.Type);
                    String xlmIn = XMLString(objLista, "SS_HC_EscalaSedacionRass_FE");
                    objAudit.TableName = "SS_HC_EscalaSedacionRass_FE";
        ...
        public int setMantenimiento(SS_HC_EscalaSedacionRass_FE objSC)
        {
                ...
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        if (objSC != null)
                        {
                            ...
                            {
                                objCount = context.SS_HC_EscalaSedacionRass_FE
                                                .Where(t =>
                                                    t.UnidadReplicacion == objSC.UnidadReplicacion
                                                    && t.IdPaciente == objSC.IdPaciente
                                                    && t.EpisodioClinico == objSC.EpisodioClinico
                            ...
                            valor = context.SS_HC_EscalaSedacionRass_FE.Count();
                            /**TRANSACCIÓN*/
                            if (objCount > 0)
                            {
                                if (objSC.Accion == "UPDATE")
                                {
                                    objSC.Version = "CCEPF448";
                            ...
                            else
                            {
                                ...
                                {
                                    var secuenciaMax = context.SS_HC_EscalaSedacionRass_FE
                                                     .DefaultIfEmpty().Max(t => t == null ? 0 : t.IdEscalaRass);
                                    objSC.IdEscalaRass = secuenciaMax + 1;
                                    objSC.Version = "CCEPF448";
        ...
        public virtual String XMLString(List<SS_HC_EscalaSedacionRass_FE> obPadre, String TablaID)
        {
