...
{
    public class EscalaBromageFERepository
    {
        ...
        public List<SS_HC_EscalaBromage_FE> listarEntidad(SS_HC_EscalaBromage_FE objSC)
        {


            List<SS_HC_EscalaBromage_FE> objLista = new List<SS_HC_EscalaBromage_FE>();
            using (var context = new ModelFormularios())
            {

                List<SS_HC_EscalaBromage_FE> objConsultas = (from t in context.SS_HC_EscalaBromage_FE
                                                                      where
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    objAudit = Control.AddAudita(new SS_HC_EscalaBromage_FE(), new SS_HC_EscalaBromage_FE(), DataKey, objAudit.Type);
                    String xlmIn = XMLString(objLista, "SS_HC_EscalaBromage_FE");
                    objAudit.TableName = "SS_HC_EscalaBromage_FE";
        ...
        public int setMantenimiento(SS_HC_EscalaBromage_FE objSC)
        {
                ...
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        if (objSC != null)
                        {
                            ...
                            {
                                objCount = context.SS_HC_EscalaBromage_FE
                                                .Where(t =>
                                                    t.UnidadReplicacion == objSC.UnidadReplicacion
                                                    && t.IdPaciente == objSC.IdPaciente
                                                    && t.EpisodioClinico == objSC.EpisodioClinico
                            ...
                            else
                            {
                                if (objSC.Accion == "NUEVO")
                                {
                                    ...
                                    var secuenciaMax = context.SS_HC_EscalaBromage_FE
                                                     .DefaultIfEmpty().Max(t => t == null ? 0 : t.IdEscalaBromage);
                                    objSC.IdEscalaBromage = secuenciaMax + 1;
                                    objSC.Version = "CCEPF446";
        ...
        public virtual String XMLString(List<SS_HC_EscalaBromage_FE> obPadre, String TablaID)
        {
