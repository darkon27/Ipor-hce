...
{
    public class VigilanciaCateterPerifericoFERepository
    {
        ...
        public List<SS_HC_ENFER_VIGI_CateterPeriferico_FE> listarSS_HC_ENFER_VIGI_CateterPeriferico_FE(SS_HC_ENFER_VIGI_CateterPeriferico_FE objSC)
        {
            List<SS_HC_ENFER_VIGI_CateterPeriferico_FE> objLista = new List<SS_HC_ENFER_VIGI_CateterPeriferico_FE>();
            using (var context = new ModelFormularios())
            {
                List<SS_HC_ENFER_VIGI_CateterPeriferico_FE> objConsultas = (from t in context.SS_HC_ENFER_VIGI_CateterPeriferico_FE
                                                                            where
                ...
                using (var contextEnty = new WEB_ERPSALUDEntities())
                {
                    ...
                    objAudit = Control.AddAudita(new SS_HC_ENFER_VIGI_CateterPeriferico_FE(), new SS_HC_ENFER_VIGI_CateterPeriferico_FE(), DataKey, objAudit.Type);
                    String xlmIn = XMLString(objLista, "SS_HC_ENFER_VIGI_CateterPeriferico_FE");
                    objAudit.TableName = "SS_HC_ENFER_VIGI_CateterPeriferico_FE";
        ...
        public int setMantenimiento(SS_HC_ENFER_VIGI_CateterPeriferico_FE objSC)
        {
                ...
                using (var dbContextTransaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        if (objSC != null)
                        {
                            ...
                            {
                                objCount = context.SS_HC_ENFER_VIGI_CateterPeriferico_FE
                                                .Where(t =>
                                                    t.UnidadReplicacion == objSC.UnidadReplicacion
                                                    && t.IdPaciente == objSC.IdPaciente
                                                    && t.EpisodioClinico == objSC.EpisodioClinico
                            ...
                            valor = context.SS_HC_ENFER_VIGI_CateterPeriferico_FE.Count();
                            /**TRANSACCIÓN*/
                            if (objCount > 0)
                            {
                                if (objSC.Accion == "UPDATE")
                                {
                                    objSC.Version = "CCEPF422";
                            ...
                            else
                            {
                                ...
                                {
                                    var secuenciaMax = context.SS_HC_ENFER_VIGI_CateterPeriferico_FE
                                                     .DefaultIfEmpty().Max(t => t == null ? 0 : t.IdCateterPeriferico);
                                    objSC.IdCateterPeriferico = secuenciaMax + 1;
                                    objSC.Version = "CCEPF422";
        ...
        public virtual String XMLString(List<SS_HC_ENFER_VIGI_CateterPeriferico_FE> obPadre, String TablaID)
        {
