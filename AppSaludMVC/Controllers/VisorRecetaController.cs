...
namespace AppSaludMVC.Controllers
{
    ...
    public class VisorRecetaController : ControllerGeneral
    {

        public VisorRecetaController()
        {
        }

        public System.Web.Mvc.ActionResult Index()
        {
            Log.Information("VisorRecetaController - Index");
        ...
        {
            Log.Information("VisorCopiadoController - VisorFormularios");
            Log.Debug("unidadReplicacion {A} , paciente {B},Clinico {C} , Atencion {D},Form {D} , Accion {E}",
                unidadReplicacion, paciente, Clinico, Atencion, Form, Accion);
        ...
        {
            Log.Information("VisorRecetaController - VisorHCEReportes");
            Log.Debug("idPaciente {A} , idOrdenAtencion {B},codigoOA {C} , Accion {D},nombrePaciente {E} , Form {F},FormContainer {G}",
                idPaciente, idOrdenAtencion, codFormato, codigoOA, Accion, nombrePaciente, Form, FormContainer);
        ...
        {
            Log.Information("VisorRecetaController - eventoLoadReporteTest");
        ...
        public System.Web.Mvc.ActionResult eventoLoadReporte(string containerId, string text)
        {
            ...
            {
                Log.Information("VisorRecetaController - Bloque Catch ");
        ...
        public PartialViewResult VisorHCExternoManuales(int idPaciente, int idOrdenAtencion,
            string codFormato, string codigoOA, string Accion, string nombrePaciente, string Form, string FormContainer)
        {
            Log.Information("VisorRecetaController - VisorHCExternoManuales");
            Log.Debug("idPaciente {A} , idOrdenAtencion {B},codigoOA {C} , Accion {D},nombrePaciente {E} , Form {F},FormContainer {G}",
                idPaciente, idOrdenAtencion, codFormato, codigoOA, Accion, nombrePaciente, Form, FormContainer);
            var objMiscl = new MA_MiscelaneosDetalle();
            var aplica = ENTITY_GLOBAL.Instance.APLICATIVOID;
        ...
        {
            Log.Information("VisorRecetaController - ArbolDocHCEManuales");
            Log.Debug("formato {A} , codigoOA {B},idPaciente {C} , idOrdenAtencion {D},tipoAtencion {E} , especialidad {F},medico {G},unidadServ {G},diagnostico {G}",
                formato, codigoOA, idPaciente, idOrdenAtencion, tipoAtencion, especialidad, medico, unidadServ, diagnostico);
            ...
            {
                Log.Information("VisorRecetaController - ArbolDocHCEManuales");
                Log.Debug("Exception {A} ", ex);
                throw ex;
            }
        }

        public System.Web.Mvc.ActionResult getSeleccionMedicoEsp_DocHC(String MODO, int persona, String cmp,
            Nullable<int> especialidad, String nombre, String idWindow)
        {
            Log.Information("VisorRecetaController - getSeleccionMedicoEsp_DocHC");
            Log.Debug("MODO {A} , persona {B}, cmp {C},especialidad {D} , nombre {E},idWindow {F} ", MODO, persona, cmp, especialidad, nombre, idWindow);
        ...
        {
            Log.Information("VisorRecetaController - getSeleccionDiagnostico_DocHC");
            Log.Debug("MODO {A} , id {B}, cod {C},descripcion {D},idWindow {E} ", MODO, id, cod, descripcion, idWindow);
            USUARIO obj = new USUARIO();
            obj.ACCION = MODO;
            var win = X.GetCmp<Window>(idWindow);
            if (win != null)
        ...
        {
            Log.Information("VisorRecetaController - getSeleccionCatUnidadServicio_DocHC");
            Log.Debug("MODO {A} , id {B}, codigo {C},descripcion {D},idWindow {E} ", MODO, id, codigo, descripcion, idWindow);
            USUARIO obj = new USUARIO();
            obj.ACCION = MODO;
            var win = X.GetCmp<Window>(idWindow);
            if (win != null)
        ...
        {
            Log.Information("VisorRecetaController - getSeleccionCatUnidadServicio_DocHC");
            Log.Debug("Accion {A} , nombre {B}, Form {C},FormContainer {D}", Accion, nombre, FormContainer);
            var objMiscl = new MA_MiscelaneosDetalle();
            var aplica = ENTITY_GLOBAL.Instance.APLICATIVOID;
            var model = new V_EpisodioAtenciones();
        ...
        {
            Log.Information("VisorRecetaController - ReturnFindMedicamento");
            Log.Debug("modo{A},  linea{B},  familia{C},  subFamilia{D}, descripcion{E},  descripLinea{F},  descripFamilia{G},idWindow {H}",
                modo, linea, familia, subFamilia, descripcion, descripLinea, descripFamilia, idWindow);
        ...
        public System.Web.Mvc.ActionResult VariableSession(String Variables, string text)
        {
            ...
            {
                Log.Information("VisorRecetaController - VariableSession");
                Log.Debug("Variables{A},  text{B}", Variables, text);
            ...
            String idTipoCuidadoPreventivo)
        {
            try
            {
                Log.Information("VisorRecetaController - CCEP0302_storeRiegoDetalle");
                Log.Debug("cuidadoPreventivo{A},  fechaSeg{B},  tipoListado{C},  idTipoCuidadoPreventivo{D}",
                    cuidadoPreventivo, fechaSeg, tipoListado, idTipoCuidadoPreventivo);
            ...
            catch (Exception ex)
            {
                ...
                Log.Information("VisorRecetaController - CCEP0302_storeRiegoDetalle - ERROR");
                Log.Debug("Exception{A}", ex);
        ...
        {
            Log.Information("VisorRecetaController - GuardaTemporalDB");
            Log.Debug("Variables{A},  text{B}",
                Variables, text);
            ...
            else
            {
                ...
                {
                    Log.Information("VisorRecetaController - GuardaTemporalDB - Exception");
                    Log.Debug("Exception{A}", ex.Message);
                    //EventLog.GenerarLogError(ex);
                    var sqlException = ex.InnerException as SqlException;
                    var detalle = new MA_MiscelaneosDetalle();
                    detalle.ACCION = "ERRORES";
        ...
        public System.Web.Mvc.ActionResult GuardaTemporalPrevencion(String Variables, string text)
        {
            try
            {
                Log.Information("VisorRecetaController - GuardaTemporalPrevencion");
                Log.Debug("Variables{A},  text{B}",
                    Variables, text);
                if ((Variables != null) || (Variables.Length > 0))
                {
        ...
        {
            Log.Information("VisorRecetaController - DobleClicGrilla");
            Log.Debug("Name{A},  selection{B},  Mode{B}",
                Name, selection, Mode);
            SelectedRowCollection src = JSON.Deserialize<SelectedRowCollection>(selection);
            List<int> omitIds = new List<int>();
            foreach (SelectedRow row in src)
        ...
        {
            Log.Information("VisorRecetaController - AsignarConsultorTelesalud");
            Log.Debug("MODO{A},  usuario{B}",
                MODO, usuario);
            //var field = X.GetCmp<TreePanel>("treeMain");
            //field.RemoveAll();
            ENTITY_GLOBAL.Instance.objModel = null;
        ...
        {
            Log.Information("VisorRecetaController - postWindowProxima");
            Log.Debug("id{A}",
                id);
        ...
        {
            Log.Information("VisorRecetaController - getGrillaEspecialidadMedico");
            Log.Debug("start {A} , limit {B},cmp {C} , nombrecompleto {D},nroregespecialidad {E} , idespecialidad {F},estado {G} , tipoBuscar {H}",
        ...
        {
            Log.Information("VisorRecetaController - getSeleccionMedico");
            Log.Debug("MODO {A} , persona {B},cmp {C} , nombre {D},idWindow {E} ",
                MODO, persona, cmp, nombre, idWindow);
            USUARIO obj = new USUARIO();
            obj.ACCION = MODO;
            var win = X.GetCmp<Window>(idWindow);
            if (win != null)
        ...
        {
            Log.Information("VisorRecetaController - getSeleccionMedicoDos");
            Log.Debug("MODO {A} , persona {B},cmp {C} , nombre {D},idWindow {E} ",
                MODO, persona, cmp, nombre, idWindow);
            USUARIO obj = new USUARIO();
            obj.ACCION = MODO;
        ...
        {
            Log.Information("VisorRecetaController - seleccionadorDiagnostico");
            Log.Debug("accionSeleccion {A} , accionListado {B}",
        ...
        {
            Log.Information("VisorRecetaController - getGrillaDiagnostico");
            Log.Debug("start {A} , limit {B},descripcion {C} , codigo {D},estado {E} , tipoBuscar {F}",
                start, limit, descripcion, codigo, estado, tipoBuscar);
        ...
        {
            Log.Information("VisorRecetaController - getGrillaUnidadServicioSelec");
            Log.Debug("start {A} , limit {B},descripcion {C} , codigoPadre {D},tipoBuscar {E}",
                start, limit, descripcion, codigoPadre, tipoBuscar);
        ...
        {
            Log.Information("VisorRecetaController - VisorDocumentosHCE");
            Log.Debug("codigoFile {A} , titulo {B},nombreFile {C} , extencionFile {D},Accion {E}, Form {F},FormContainer {G}",
                codigoFile, titulo, nombreFile, extencionFile, Accion, Form, FormContainer);
            var aplica = ENTITY_GLOBAL.Instance.APLICATIVOID;
            ...
            string Form, string Accion, string MODO)
        {
            try
            {
                Log.Information("VisorRecetaController - DialogVerFile");
                Log.Debug("pathAuxDefault {A} , nombreArchivo {B},Form {C} , Accion {D},MODO {E}",
                    pathAuxDefault, nombreArchivo, Form, Accion, MODO);
            ...
            {
                Log.Information("VisorRecetaController - VisorDocumentosHCE - Exception");
                Log.Debug("Exception {A}", ex.Message);
            }
            return this.Direct();
        }
        ...
        {
            Log.Information("VisorRecetaController - ListadoPacienteEpisodioAtencion");
            Log.Debug("start {A} , limit {B},proceso {C} , profSalud {D},txtbuscar {E} , fechaDesde {F},fechaHasta {G} , tipoBuscar {H}, codigooa {I}, accion {J}",
                   start, limit, proceso, profSalud, txtbuscar, fechaDesde, fechaHasta, tipoBuscar, codigooa, accion);
        ...
        public System.Web.Mvc.ActionResult getColumnasCronologia(String opcion, String accion)
        {
            try
            {
                Log.Information("VisorRecetaController - getColumnasCronologia");
                Log.Debug("opcion {A} ,  accion {B}",
                       opcion, accion);
                var detalle = new MA_MiscelaneosDetalle();
        ...
        {
            Log.Information("VisorRecetaController - ListadoPacienteEpisodio");
            Log.Debug("start {A} , limit {B},Name {C} , selection {D},Mode {E}",
                   start, limit, Name, selection, Mode);
        ...
        {
            Log.Information("VisorRecetaController - CCEP9923_View");
            ...
            if (ENTITY_GLOBAL.Instance.IDSOLICITUDMED > 0)
            {
                ...
                //ObjSP.IdSolicitudProducto = (int)ENTITY_GLOBAL.Instance.IDSOLICITUDMED;
                //ObjSP.Accion = "NDOCUMENTO";
                List<SS_FA_SolicitudProducto> result = svcSS_FA_SolicitudProducto.ListarSolicitudProductoListar(ObjSP);
                foreach (SS_FA_SolicitudProducto objEntity in result)
                {
                    var fieldCod4 = X.GetCmp<TextField>("txtDocumento");
                    fieldCod4.SetValue(objEntity.NumeroDocumento.ToString());

                    var fieldCod5 = X.GetCmp<TextArea>("txtObservacion");
                    fieldCod5.SetValue(objEntity.Observacion.ToString());
                }

                var fieldCod6 = X.GetCmp<TextField>("txtIdSolicitudProducto");
                fieldCod6.SetValue(ENTITY_GLOBAL.Instance.IDSOLICITUDMED);
            }

            return View("DevolucionMed/CCEP9923_View", "");
        }
        ...
        {
            Log.Information("VisorRecetaController - PanelEast");
            Log.Debug("containerId {A}",
                   containerId);
        ...
        {
            Log.Information("VisorRecetaController - ComboAlmacenServicios");
            Log.Debug("Usuario {A}, Sucursal {B}, IdAgente {C}", Usuario, Sucursal, IdAgente);
        ...
        {
            Log.Information("VisorRecetaController - PanelNorth");
            Log.Debug("containerId {A}", containerId);
            ////////////////////////
        ...
        {
            Log.Information("VisorRecetaController - ListadoTratamientoActivoPaciente");
            Log.Debug("idPaciente {A}, fecha {B}", idPaciente, fecha);
            var detalle = new MA_MiscelaneosDetalle();
            detalle.ACCION = "TRATACTIVO";
            int paciente = (int)ENTITY_GLOBAL.Instance.PacienteID;
        ...
        {
            Log.Information("VisorRecetaController - ListadoPreexistenciasPaciente");
            Log.Debug("idPaciente {A}, fecha {B}", idPaciente, fecha);
        ...
        {
            Log.Information("VisorRecetaController - PanelSouth");
            Log.Debug("containerId {A}", containerId);
            var objVistaSeguridad = new SS_CHE_VistaSeguridad();
            objVistaSeguridad.Sistema = ENTITY_GLOBAL.Instance.APPCODIGO;
            objVistaSeguridad.CodigoAgente = ENTITY_GLOBAL.Instance.USUARIO;
        ...
        {
            Log.Information("VisorRecetaController - ShowProperties");
            Log.Debug("model {A}, containerId {B}", model, containerId);
        ...
        {
            Log.Information("VisorRecetaController - LoadFormatos");
            Log.Debug("containerId {A}, text {B}", containerId, text);
            string WEBNUEVO = "";
            string WEBMODIFICAR = "";
            ...
            if (resulListado.Count > 0)
            {
                ...
                else
                {
                    ...
                    if (objVistaSeguridad.CodigoFormato == "CCEP9918")
                    {
        ...
        {
            Log.Information("VisorRecetaController - LoadFormatosArbol");
            Log.Debug("containerId {A}, opc {B}", containerId, opc);
            string WEBNUEVO = "";
            string WEBMODIFICAR = "";
        ...
        {
            Log.Information("VisorRecetaController - EditCentral");
            Log.Debug("selection {A}, text {B}", selection, text);
            var objMiscl = new MA_MiscelaneosDetalle();
            var aplica = ENTITY_GLOBAL.Instance.APLICATIVOID;
            var model = new SEGURIDADCONCEPTO();
            string estado = "PanelCentralBlanco";
        ...
        {
            Log.Information("VisorRecetaController - GetTreeView");
            Node root1 = new Node();
            root1.Text = "ds";
            bool submit = true;
            for (int i = 0; i < 10; i++)
        ...
        {
            Log.Information("VisorRecetaController - GetListarBusquedaServicios");
            Log.Debug("start {A} , limit {B},tipofiltro {C} , filtro {D},tipoBuscar {E} , Linea {F},Familia {G} , Stock {H}",
           start, limit, tipofiltro, filtro, tipoBuscar, Linea, Familia, Stock);
            ENTITY_GLOBAL.Instance.GRUPO = "";
        ...
        {
            Log.Information("VisorRecetaController - ArbolMacroProcesos");
            Log.Debug("node {A}, filtro {B}", node, filtro);
            NodeCollection nodes = new Ext.Net.NodeCollection();
            var objVistaSeguridad = new SS_CHE_VistaSeguridad();
            //OBS:ADD de seguridad
        ...
        {
            Log.Information("VisorRecetaController - GetTreeViewChildRitgh");
            Log.Debug("node {A}, filtro {B}", node, filtro);
            NodeCollection nodes = new Ext.Net.NodeCollection();
        ...
        {
            Log.Information("VisorRecetaController - GetTreeViewChild");
            Log.Debug("node {A}", node);
            NodeCollection nodes = new Ext.Net.NodeCollection();
            if (node == "WA")
            {
                var entidaLocal = new SEGURIDADGRUPO();
        ...
        {
            Log.Information("VisorRecetaController - GetTreeViewCuidadosPrevSegRiesgos");
            Log.Debug("node {A}, tipo {B}", node, tipo);
        ...
        {
            Log.Information("VisorRecetaController - getSeguridadOpciones");
            Log.Debug("filtro {A}", filtro);
        ...
        {
            Log.Information("VisorRecetaController - RegistraPagina");
            String comillas = @"""";
            System.Text.StringBuilder htmlOutput = new System.Text.StringBuilder();
        ...
        {
            Log.Information("VisorRecetaController - cambioSeleccionCronologias");
        ...
        {
            Log.Information("VisorRecetaController - validarCambiosFormulario");
            Log.Debug("containerId {A},text {A} ", containerId, text);
            if (ENTITY_GLOBAL.Instance.INDICA_SELECCIONCRONOLOGIAS == 1)
            {
                ENTITY_GLOBAL.Instance.UnidadReplicacion = ENTITY_GLOBAL.Instance.UNIDREPLICACION_TEMP;
                ENTITY_GLOBAL.Instance.PacienteID = ENTITY_GLOBAL.Instance.IDPACIENTE_TEMP;
        ...
        {
            Log.Information("VisorRecetaController - validarCambiosFormularioInicio");
            Log.Debug("containerId {A},text {A} ", containerId, text);
            if (ENTITY_GLOBAL.Instance.INDICA_SELECCIONCRONOLOGIAS == 1)
            {
                ENTITY_GLOBAL.Instance.UnidadReplicacion = ENTITY_GLOBAL.Instance.UNIDREPLICACION_TEMP;
        ...
        {
            Log.Information("VisorRecetaController - add_delete_DataSeleccFavoritos");
            Log.Debug("text {A} , id {B},numero {C} , valor {D}",
                 text, id, numero, valor);
        ...
        {
            Log.Information("VisorRecetaController - eventoMantenFavoritoRegistro");
            Log.Debug("MODO {A} , idFavorito {B},idNumero {C} , ItemSeleccionado {D},indicaItem {E} ",
                 MODO, idFavorito, idNumero, ItemSeleccionado, indicaItem);
        ...
        {
            Log.Information("VisorRecetaController - save_Addfavorito");
            Log.Debug("objSave {A} , MODO {B},idWindow {C} , indicaItem {D}",
                 objSave, MODO, idWindow, indicaItem);
        ...
        {
            Log.Information("VisorRecetaController - getFiltroFavoritos");
        ...
        {
            Log.Information("VisorRecetaController - addRecursosValidacion");
            Log.Debug("data {A} , indica {B}",
                 data, indica);
            /*******SET PROP. FORMULARIO*****************************/
            Session["MENSAJES_VALFORM"] = null;
        ...
        {
            Log.Information("VisorRecetaController - eventoVerFile");
            Log.Debug("nombreArchivo {A} , idRegistro {B},Form {C} , Accion {D}",
                  nombreArchivo, idRegistro, Form, Accion);
        ...
        {
            Log.Information("VisorRecetaController - eventoAbout");
            Log.Debug("indicador {A} ",
           indicador);
        ...
        public System.Web.Mvc.ActionResult EstadoClinico(Nullable<int> selection, string accion)
        {
            try
            {
                Log.Information("VisorRecetaController - EstadoClinico");
                Log.Debug("selection {A},accion {B}  ",
               selection, accion);
                int registro = -1;
                string mensaje = "";
                if (accion == "Nuevo")
        ...
        {
            Log.Information("VisorRecetaController - EstadoClinicoMedicamento");
            Log.Debug("selection {A},accion {B}  ",
            selection, accion);
            Label labelX = X.GetCmp<Ext.Net.Label>("lblNumEpsiodioHCE");
            Button btnNuevo = X.GetCmp<Ext.Net.Button>("btnNewEpisodio_HCE");
        ...
        {
            Log.Information("VisorRecetaController - EstadoClinicoMedicamento");
            Log.Debug("MODO {A},paciente {B} ,nombre {C}, codigooa {D}",
            MODO, paciente, nombre, codigooa);
            ...
            string tipoConsulta, string tipoEstado, string idPaciente, string tipoBuscar)
        {
            ...
            try
            {
                Log.Information("VisorRecetaController - GrillaListadoAtencionPacientesContinuar");
                Log.Debug("start {A} , limit {B},txtFecha1 {C} , txtFecha2 {D},txtCodigoOA {E} , txtPaciente {F},tipoConsulta {G} , tipoEstado {H}, idPaciente {I}, tipoBuscar {J}",
                start, limit, txtFecha1, txtFecha2, txtCodigoOA, txtPaciente, tipoConsulta, tipoEstado, idPaciente, tipoBuscar);
        ...
        public System.Web.Mvc.ActionResult POSaludDescansoMedico(int PacienteID, int dias, String regla, String arrays)
        {
            ...
            if (PARAM == "S")
            {
                try
                {
                    Log.Information("VisorRecetaController - POSaludDescansoMedico");
                    Log.Debug("PacienteID {A} , dias {B},regla {C} , arrays {D}",
                    PacienteID, dias, regla, arrays);
        ...
        public System.Web.Mvc.ActionResult POSaludControlGinecologia(int PacienteID, int valor, String regla, String arrays)
        {
            ...
            if (PARAM == "S")
            {
                try
                {
                    Log.Information("VisorRecetaController - POSaludControlGinecologia");
                    Log.Debug("PacienteID {A} , valor {B},regla {C} , arrays {D}",
        ...
        public System.Web.Mvc.ActionResult POSaludEmergenciaEspera(int PacienteID, int valor, String regla, String arrays)
        {
            ...
            if (PARAM == "S")
            {
                Log.Information("VisorRecetaController - POSaludEmergenciaEspera");
                Log.Debug("PacienteID {A} , valor {B},regla {C} , arrays {D}",
                PacienteID, valor, regla, arrays);
                try
                {
        ...
        public System.Web.Mvc.ActionResult POSaludExamenLaboratorio(int PacienteID, int valor, String regla, String arrays)
        {
            ...
            if (PARAM == "S")
            {
                try
                {
                    Log.Information("VisorRecetaController - POSaludExamenLaboratorio");
                    Log.Debug("PacienteID {A} , valor {B},regla {C} , arrays {D}",
                    PacienteID, valor, regla, arrays);
                    int tiempoDescanso = 0;
                    /******* MISCELANEOS AQUILES INFORMACION*****************************/
        ...
        public System.Web.Mvc.ActionResult CCEX0001_View()
        {
            ...
            try
            {
                Log.Information("VisorRecetaController - CCEX0001_View");
        ...
        public System.Web.Mvc.ActionResult CCEP2010_View()
        {
            try
            {
                Log.Information("VisorRecetaController - CCEP2010_View");
        ...
        public System.Web.Mvc.ActionResult Save_EvolucionObjetiva(SS_HC_EvolucionObjetiva objSave, String HoraIngreso)
        {
            ...
            Log.Information("VisorRecetaController - Save_EvolucionObjetiva");
            Log.Debug("objSave {A} , HoraIngreso {B}",
            objSave, HoraIngreso);
            if (ENTITY_GLOBAL.Instance.EpisodioClinicoEstado != 1)
            {
                X.Msg.Alert("Ventana de Mensajes ", "Por favor seleccione episodio clínico... ").Show();
        ...
        public System.Web.Mvc.ActionResult CCEP0055_View()
        {
            try
            {
                Log.Information("VisorRecetaController - CCEP0055_View");
        ...
        {
            Log.Information("VisorRecetaController - CCEP0055_ANTEC_FAMILIARES_MAIN");
            var LocalEnty = new SS_HC_Anamnesis_AFAM();
            var Listar = new List<SS_HC_Anamnesis_AFAM>();
            LocalEnty.Accion = "LISTAR";
            if (ENTITY_GLOBAL.Instance.ESTADOFORMULARIO_ACCION == "VISTA")
        ...
        {
            Log.Information("VisorRecetaController - CCEP0055_ANTEC_FAMILIARES");
            Log.Debug("secuenciaFamilia {A}",
            secuenciaFamilia);
            var LocalEnty = new MA_MiscelaneosDetalle();
            var Listar = new List<MA_MiscelaneosDetalle>();
        ...
        {
            Log.Information("VisorRecetaController - Save_Antic_FamiliaresAlt");
            Log.Debug("objAnamEA {A} , selection {B},selectionMain {C} , text {D}",
            objAnamEA, selection, selectionMain, text);
            String mensaje = "";
        ...
        {
            Log.Information("VisorRecetaController - addEliminar_Antic_FamiliaresMainAlt");
            Log.Debug("MODO {A} , secuenciaFamilia {B}",
            MODO, secuenciaFamilia);
        ...
        {
            Log.Information("VisorRecetaController - addDataUpdate_Antic_FamiliaresAlt");
            Log.Debug("MODO {A} , data {B},tipo {C}",
            MODO, data, tipo);
            if (data != null)
            {
                List<MA_MiscelaneosDetalle> objLista = null;
        ...
        {
            Log.Information("VisorRecetaController - addEliminar_Antic_FamiliaresAlt");
            Log.Debug("MODO {A} , codigo {B},secuencia {C} , secuenciaFam {D}, tipo {E}",
            MODO, codigo, secuencia, secuenciaFam, tipo);
            List<SS_HC_Anamnesis_AFAM_Enfermedad> dataDelete = null;
            if ((codigo + "").Length > 0)
            {
        ...
        {
            Log.Information("VisorRecetaController - HCEReportesPartialView");
            Log.Debug("Accion {A}",
            Accion);
        ...
        {
            Log.Information("VisorRecetaController - HCEReportesBitacoraView");
            Log.Debug("containerId {A}",
            containerId);
            ENTITY_GENERALHCE objReporte = new ENTITY_GENERALHCE
            {
        ...
        {
            Log.Information("VisorRecetaController - HCEReportesView");
        ...
        {
            Log.Information("VisorRecetaController - HCEReportesView_total");
        ...
        {
            Log.Information("VisorRecetaController - setCodigoFormatosPaginas");
            Log.Debug("indicadorEtapa {A} , opcionView {B}",
        ...
        {
            Log.Information("VisorRecetaController - initLoadFormatos");
            Log.Debug("container {A}",
            container);
            string containerId = "";
            string text = "0";
        ...
        {
            Log.Information("VisorRecetaController - cargarPermisosFormato");
            Log.Debug("activo {A}",
            activo);
        ...
        public System.Web.Mvc.ActionResult visorHCECopiarFormularios(string Accion, string nombrePaciente, string Form, string FormContainer)
        {
            Log.Information("VisorRecetaController - visorHCECopiarFormularios");
            Log.Debug("Accion {A} , nombrePaciente {B},Form {C} , FormContainer {D}",
            Accion, nombrePaciente, Form, FormContainer);
            Session["COPIA_FORMULARIO"] = "INACTIVA";
        ...
        {
            Log.Information("VisorRecetaController - eventoRegistroHistorialForm");
            Log.Debug("Accion {A} , nombrePaciente {B},Form {C} , FormContainer {D}",
            Accion, nombrePaciente, Form, FormContainer);
            var objMiscl = new MA_MiscelaneosDetalle();
            var aplica = ENTITY_GLOBAL.Instance.APLICATIVOID;
        ...
        /***/
        public System.Web.Mvc.ActionResult eventoRegistroHistoricoFormulario(string formato, string accion, string idWindow, Nullable<long> idEpiAtencionCopy,
            Nullable<int> idEpiClinicoCopy, string UnidadReplicacionCopy, String data)
        {
            Log.Information("VisorRecetaController - eventoRegistroHistoricoFormulario");
            Log.Debug("formato {A} , accion {B},idWindow {C} , idEpiAtencionCopy {D}, idEpiClinicoCopy {E}, UnidadReplicacionCopy {F}, data {G}",
            formato, accion, idWindow, idEpiAtencionCopy, idEpiClinicoCopy, UnidadReplicacionCopy, data);
            if ("COPIAR" == accion)
            {
        ...
        public System.Web.Mvc.ActionResult eventoAgregarHistoricoFormulario(string formato, string accion, string idWindow,
            Nullable<long> idEpiAtencionCopy, Nullable<int> idEpiClinicoCopy, string UnidadReplicacionCopy, String data)
        {
            Log.Information("VisorRecetaController - eventoAgregarHistoricoFormulario");
            Log.Debug("formato {A} , accion {B},idWindow {C} , idEpiAtencionCopy {D}, idEpiClinicoCopy {E}, UnidadReplicacionCopy {F}, data {G}",
            formato, accion, idWindow, idEpiAtencionCopy, idEpiClinicoCopy, UnidadReplicacionCopy, data);
        ...
        {
            Log.Information("VisorRecetaController - getObjetoMiscelaneoFormulario_de_Historial");
            Log.Debug("data {A} , accion {B},codigoForm {C} , dataValidacion {D}",
            data, accion, codigoForm, dataValidacion);
        ...
        {
            Log.Information("VisorRecetaController - getCodigoElementoForm");
            Log.Debug("segmentoCadena {A} , codigoForm {B}",
            segmentoCadena, codigoForm, codigoForm);
            String codigo = "";
            try
            {
        ...
        {
            Log.Information("VisorRecetaController - getIndicaFormatosCopiar");
            Log.Debug("codigoFormato {A}",
            codigoFormato);
        ...
        {
            Log.Information("VisorRecetaController - ArbolMacroProcesosBitacora");
            Log.Debug("node {A} , filtro {B}",
            node, filtro);
            NodeCollection nodes = new Ext.Net.NodeCollection();
            var objVistaSeguridad = new SS_CHE_VistaSeguridad();
            if (node == "root")
        ...
        {
            Log.Information("VisorRecetaController - LoadFormatoBitacora");
            Log.Debug("containerId {A} , node {B}",
            containerId, node);
        ...
        {
            Log.Information("VisorRecetaController - ExamenesAdicionales");
            Log.Debug("MODO {A}",
            MODO);
        ...
        {
            Log.Information("VisorRecetaController - getGrillaNotificacionDetalle");
            Log.Debug("start {A} , limit {B},desc {C} , tipoBuscar {D}, estado {E}",
            start, limit, desc, tipoBuscar, estado);
            ENTITY_GLOBAL.Instance.GRUPO = "";
            //ConsultaCita();
        ...
        {
            Log.Information("VisorRecetaController - getPaneles");
            Log.Debug("accion {A}",
            accion);
            try
            {
        ...
        {
            Log.Information("VisorRecetaController - cargar_planf");
            Log.Debug("idNanda {A}",
            idNanda);
        ...
        public System.Web.Mvc.ActionResult getGrillaFactores(int start, int limit, int desc, string tipoBuscar, string estado)
        {
            Log.Information("VisorRecetaController - cargar_planf");
            Log.Debug("start {A} , limit {B},desc {C} , tipoBuscar {D}, estado {E}",
            start, limit, desc, tipoBuscar, estado);
        ...
        {
            Log.Information("VisorRecetaController - AdicionarFactor");
            Log.Debug("MODO {A} , IdNanda {B}",
            MODO, IdNanda);
        ...
        {
            Log.Information("VisorRecetaController - getGrillaFactorRelacionadoNanda");
            Log.Debug("start {A} , limit {B},IdNanda {C} , IdFactorRelacionado {D}, tipoBuscar {E}",
            start, limit, IdNanda, IdFactorRelacionado, tipoBuscar);
            Boolean indicaValidacionForm = false;
            try
            {
                ENTITY_GLOBAL.Instance.GRUPO = "";
        ...
        public System.Web.Mvc.ActionResult eventoAgregarHistoricoFormulario2(string formato, string accion, string idWindow,
            Nullable<long> idEpiAtencionCopy, Nullable<int> idEpiClinicoCopy, string UnidadReplicacionCopy, String data)
        {
            Log.Information("VisorRecetaController - eventoAgregarHistoricoFormulario2");
            Log.Debug("formato {A} , accion {B},idWindow {C} , idEpiAtencionCopy {D}, idEpiClinicoCopy {E}, UnidadReplicacionCopy {F}, data {G}",
            formato, accion, idWindow, idEpiAtencionCopy, idEpiClinicoCopy, UnidadReplicacionCopy, data);
            List<MA_MiscelaneosDetalle> dataSave = null;
            var varPasarDetalle = new MA_MiscelaneosDetalle();
        ...
        {
            Log.Information("VisorRecetaController - eventoFirmaActoMedicoRegistro");
            Log.Debug("MODO {A} , idpaciente {B},codigooa {C} , paciente {D}, password {E}",
            MODO, idpaciente, codigooa, paciente, password);
            /****************************************************/
        ...
        {
            Log.Information("VisorRecetaController - eventoProcesoInteroperabilidad");
            Log.Debug("modo {A}",
            modo);
            List<ENTITY_MENSAJES> msgNoValido = null;
            int idResultado = 0;
        ...
        {
            Log.Information("VisorRecetaController - GrillaListadoMedicamentoPacientes");
            Log.Debug("start {A} , limit {B},txtFecha1 {C} , txtFecha2 {D}, txtPaciente {E}, tipoEstado {F}, tipoBuscar {G}",
            ...
            {
                //ENTITY_GLOBAL.Instance.GRUPO = "";
                var Listar = new List<SS_FA_SolicitudProductoDetalle>();
                var Listar2 = new List<SS_FA_SolicitudProductoDetalle>();
                var LocalEnty = new SS_FA_SolicitudProducto();
                ...
                if (SoluccionSalud.Entidades.Entidades.ENTITY_GLOBAL.Instance.IDSOLICITUDMED != null)
                {
                    LocalEnty.IdSolicitudProducto = (int)SoluccionSalud.Entidades.Entidades.ENTITY_GLOBAL.Instance.IDSOLICITUDMED;
                }
                /* if (tipoEstado == "-1")
                 {
                     LocalEnty.Estado = null;
                ...
                //  {
                LocalEnty.Accion = "LISTAR";
                if (LocalEnty.IdSolicitudProducto > 0)
                {
                    ENTITY_GLOBAL.Instance.ESTADOFORMULARIO_ACCION = "UPDATE";
                    Listar = svcSS_FA_SolicitudProducto.ListarDetalle(LocalEnty);
                }
                //if (ENTITY_GLOBAL.Instance.ESTADOFORMULARIO_ACCION == "UPDATE")
                //{
                //    Listar = svcSS_FA_SolicitudProducto.ListarDetalle(LocalEnty);
                //}
                foreach (SS_FA_SolicitudProductoDetalle objEntity1 in Listar)
                {
                    ...
                    objEntity1.LineaOrdenAtencion = Listar.Count;
                    Listar2.Add(objEntity1);
                }
                if (Listar.Count == 0)
                {
                    Listar2 = Listar;
                ...
                Session["HC_SolMedicamentoDetList"] = Listar2;
                return this.Store(Listar2, cantElementos);
            }
            catch (Exception ex)
            {
                Log.Information("VisorRecetaController - Bloque Catch ");
        ...
        //Agregando nuevo metodo para Agregar Med en Dispensación Visor Receta
        public System.Web.Mvc.ActionResult GrillaListadoMedicamentoPacientes02(int start, int limit, string txtFecha1, string txtFecha2,
        string txtPaciente, string tipoEstado, string tipoBuscar)
        {
            Log.Information("VisorRecetaController - GrillaListadoMedicamentoPacientes");
            Log.Debug("start {A} , limit {B},txtFecha1 {C} , txtFecha2 {D}, txtPaciente {E}, tipoEstado {F}, tipoBuscar {G}",
            ...
            try
            {
                ...
                if (ENTITY_GLOBAL.Instance.IDSOLICITUDMED != null)
                {
                    LocalEnty.IdSolicitudProducto = (int)ENTITY_GLOBAL.Instance.IDSOLICITUDMED;
                }
                /* if (tipoEstado == "-1")
                 {
                     LocalEnty.Estado = null;
       ...
       string txtPaciente, string tipoEstado, string tipoBuscar, string descripcion, int grupo)
        {
            Log.Information("VisorRecetaController - GrillaListadoMedicamentoPacientesAdd");
            Boolean indicaValidacionForm = false;
            int contador = 0;
            var objLista = new List<SS_FA_SolicitudProductoDetalle>();
            try
            {
                ...
                LocalEnty.Estado = getValorFiltroInt(tipoEstado);
                LocalEnty.Medicamento = descripcion;
                LocalEnty.GrupoMedicamento = grupo;
                LocalEnty.UnidadReplicacion = SoluccionSalud.Entidades.Entidades.ENTITY_GLOBAL.Instance.UnidadReplicacion;
                LocalEnty.IdPaciente = (int)SoluccionSalud.Entidades.Entidades.ENTITY_GLOBAL.Instance.PacienteID;
                LocalEnty.EpisodioClinico = (int)SoluccionSalud.Entidades.Entidades.ENTITY_GLOBAL.Instance.EpisodioClinico;
                LocalEnty.IdEpisodioAtencion = (long)SoluccionSalud.Entidades.Entidades.ENTITY_GLOBAL.Instance.EpisodioAtencion;
                ...
                if (Listar.Count > 0)
                {
                    contador = Convert.ToInt32(Listar[0].Medico);
                }
                ...
                foreach (VW_ATENCIONPACIENTEMEDICAMENTO objEntity1 in Listar)
                {
                    ...
                    obtemp.UsuarioCreacion = objEntity1.Cama;
                    objLista.Add(obtemp);
                }
        ...
        {
            Log.Information("VisorRecetaController - SelectPersonaMedicamentoEpisodioEvento");
            //ENTITY_GLOBAL.Instance.NIVEL = 1;
        ...
        public System.Web.Mvc.ActionResult CCEP9918_View()
        {
            Log.Information("VisorRecetaController - CCEP9918_View");
            ...
            if (ENTITY_GLOBAL.Instance.IDSOLICITUDMED > 0)
            {
                ENTITY_GLOBAL.Instance.ESTADOFORMULARIO_ACCION = "UPDATE";
                var fieldCod4 = X.GetCmp<TextField>("txtDocumento");
                fieldCod4.SetValue(ENTITY_GLOBAL.Instance.GuiaPedido);

                //   result = ENTITY_GLOBAL.Instance.ESTADOFORMULARIO_ACCION == "NUEVO" ? "" : result;
                ObjSP = new SS_FA_SolicitudProducto();
                ObjSP.UnidadReplicacion = ENTITY_GLOBAL.Instance.UnidadReplicacion;
                ObjSP.IdEpisodioAtencion = (long)ENTITY_GLOBAL.Instance.EpisodioAtencion;
                ObjSP.IdPaciente = (int)ENTITY_GLOBAL.Instance.PacienteID;
                ...
                {
                    var fieldCod5 = X.GetCmp<TextArea>("txtObservacion");
                    fieldCod5.SetValue(objEntity.Observacion);

                    var fieldCod6 = X.GetCmp<TextField>("txtIdSolicitudProducto");
                    fieldCod6.SetValue(objEntity.IdSolicitudProducto.ToString());

                    var fieldCod8 = X.GetCmp<TextArea>("txtIndicaciones");
                    fieldCod8.SetValue(objEntity.indicaciones);

                    var fieldCod7 = X.GetCmp<DateField>("txtEstado");
                    fieldCod7.SetValue(objEntity.Estado.ToString());
                }
            ...
            {
                var btn1 = X.GetCmp<Button>("btnagregarM");
                var btn2 = X.GetCmp<Button>("btnSelDesactivo0");
                var guardar = X.GetCmp<Button>("btnGuardar");
            ...
            else
            {
                ...
                if (PARAM == "S" && ENTITY_GLOBAL.Instance.COD_BANDEJA != "EMERGENCIA")
                {
                    ...
                    try
                    {
                        ...
                        WsEpisodio.UsuarioCreacion = "LISTALMACEN";
                    ...
                    {
                        Log.Information("VisorRecetaController - CCEP9918_View - Bloque Catch");
                        Log.Error(ex, ex.Message);
                        EventLog.GenerarLogError(ex);
                ...
                string VarAlmacen = "";
                foreach (DataRow ht_fila in dtValida.AsEnumerable())
                {
                    VarAlmacen = ht_fila["AlmacenEmergencia"].ToString();
                }
        ...
        {
            Log.Information("VisorRecetaController - Save_Solicitud");
            ...
            if (selectionArray1.Trim().Length > 0)
            {
                try
                {
                    ...
                    foreach (SS_FA_SolicitudProducto objEntity1 in ObjLista1)
                    {
                        ...
                        //  ObjSP.IdPaciente = (int)ENTITY_GLOBAL.Instance.PacienteID;
                        //   ObjSP.UnidadReplicacion = ENTITY_GLOBAL.Instance.UnidadReplicacion;
                        w_numerodocumento = ObjSP.NumeroDocumento;

                        //if (ENTITY_GLOBAL.Instance.despachoExtra)
                        //{
                        //    if (objEntity1.NumeroDocumento.Count() > 0)
                        //    {
                        //        //ObjSP.Accion = "UPDATE_RETORNO";
                        //        //ObjSP.UsuarioModificacion = ENTITY_GLOBAL.Instance.USUARIO;
                        //        //ObjSP.FechaModificacion = DateTime.Now;
                        //        int idResultadoLineaMedicamentos = 0;
                        //        var objGenralMedicamento = new SS_HCE_ConsultaExterna();
                        //        objGenralMedicamento.SecuenciaHCE = ObjSPD.UsuarioModificacion;
                        //        objGenralMedicamento.Componente = ENTITY_GLOBAL.Instance.CodigoOA;
                        //        objGenralMedicamento.Accion = (ObjSPD.UsuarioModificacion == null || ObjSPD.UsuarioModificacion == "") ? "LINEA_ENF" : "LINEA_MED";

                        //        idResultadoLineaMedicamentos = SOA_AtencionesSpring.ListaConsultaExternaEmergencia(objGenralMedicamento);
                        //    }

                        //    cabeceraMed02 = ObjSP;

                        //    int resultss = svcSS_FA_SolicitudProducto.setMantenimiento02(cabeceraMed);
                        ...
                        {
                            ObjSP.Accion = "NUEVO";
                            ObjSP.UsuarioCreacion = ENTITY_GLOBAL.Instance.USUARIO;
                            ObjSP.FechaCreacion = DateTime.Now;
                        }
                        if (objEntity1.Estado == 4)
                        {
                            ObjSP.Accion = "ANULAR";
                    ...
                    foreach (SS_FA_SolicitudProductoDetalle objEntity2 in ObjLista2)
                    {
                        ...
                        //ObjSPD.UsuarioModificacion = objEntity2.UsuarioCreacion;
                        ...
                        if (ENTITY_GLOBAL.Instance.despachoExtra)
                        {
                            ...
                            //ObjSP.UsuarioModificacion = ENTITY_GLOBAL.Instance.USUARIO;
                            //ObjSP.FechaModificacion = DateTime.Now;
                            int idResultadoLineaMedicamentos02 = 0;
                            var objGenralMedicamento02 = new SS_HCE_ConsultaExterna();
                            objGenralMedicamento02.SecuenciaHCE = ObjSPD.UsuarioModificacion;
                            objGenralMedicamento02.Componente = cabeceraMed.NumeroDocumento;
                            objGenralMedicamento02.Accion = "UPDATE_RETORNO";
                            objGenralMedicamento02.UsuarioModificacion = ENTITY_GLOBAL.Instance.USUARIO;
                            objGenralMedicamento02.FechaModificacion = DateTime.Now;

                            idResultadoLineaMedicamentos02 = SOA_AtencionesSpring.ListaConsultaExternaEmergencia(objGenralMedicamento02);
                        ...
                        if (objEntity2.Accion == "NUEVO" || objEntity2.Accion == "UPDATE")
                        {
                            ...
                            ObjSPD.UsuarioCreacion = ENTITY_GLOBAL.Instance.USUARIO;
                        ...
                        //VALIDAR LINEA DE LOS MEDICAMENTOS

                        if (ObjSPD.Accion == "NUEVO")
                        {
                            int idResultadoLineaMedicamentos = 0;
                            var objGenralMedicamento = new SS_HCE_ConsultaExterna();
                            objGenralMedicamento.SecuenciaHCE = ObjSPD.UsuarioModificacion;
                            ...
                            idResultadoLineaMedicamentos = SOA_AtencionesSpring.ListaConsultaExternaEmergencia(objGenralMedicamento);
                            if (ObjSPD.UsuarioModificacion == null || ObjSPD.UsuarioModificacion == "")
                            {
                                contadorEnf = contadorEnf == 0 ? idResultadoLineaMedicamentos + 1 : contadorEnf + 1;
                            }
                            ObjSPD.SecuencialHCE = objEntity2.UsuarioModificacion;
                            //ObjSPD.IdOrdenAtencion = (ObjSPD.UsuarioModificacion == null || ObjSPD.UsuarioModificacion == "") ? contadorEnf : idResultadoLineaMedicamentos;
                            // agrgado 20211228 
                            ObjSPD.IdOrdenAtencion = ENTITY_GLOBAL.Instance.IdOrdenAtencion;
                            ObjSPD.Version = objEntity2.Version; //ENTITY_GLOBAL.Instance.LineaOrdenAtencion.ToString();
                            List<SS_HC_Medicamento_FE> LISTNEWCAB = new List<SS_HC_Medicamento_FE>();
                            int flat = 2;
                            int idResultados = 0;

                            idResultados = SvcMedicamentoFE.setMantenimientoNotificaciones(flat, LISTNEWCAB, ObjSPD);
                        }
                    ...
                    if (Session["HC_SolMedicamentoDetListDELETE"] != null)
                    {
                        ...
                        foreach (var result2 in listaDel)
                        {
                            ...
                            if (result2.Accion == "DELETE")
                            {
                                if (result2.TipoComponente == "M")
                                {
                                    var objElimna = new SS_HCE_RecetaConsultaExterna();

                                    objElimna.TIPOORDENATENCION = 2500;
                                    objElimna.IdOrdenAtencion = ENTITY_GLOBAL.Instance.IdOrdenAtencion;
                                    objElimna.LineaOA = ENTITY_GLOBAL.Instance.LineaOrdenAtencion;
                                    objElimna.SecuenciaHCE = result2.SecuencialHCE;

                                    flat = SOA_AtencionesSpring.ListaRecetaConsultaExternaEmergencia(objElimna);
                                ...
                                else // medicamentos
                                {
                                    ...
                                    objElimna.TIPOORDENATENCION = 2600;
                                    objElimna.IdOrdenAtencion = ENTITY_GLOBAL.Instance.IdOrdenAtencion;
                                    objElimna.LineaOA = ENTITY_GLOBAL.Instance.LineaOrdenAtencion;
                                    objElimna.SecuenciaHCE = result2.SecuencialHCE;
                    ...
                    objGenral.Componente = ENTITY_GLOBAL.Instance.CodigoOA;
                    ...
                    {
                        objGenral.Accion = "HEADER_DESPACHO";
                        result = SOA_AtencionesSpring.ListaConsultaExternaEmergencia(objGenral);
                        ENTITY_GLOBAL.Instance.IDSOLICITUDMED = result;
                    }
                    else
                    {
                        result = Convert.ToInt32(ENTITY_GLOBAL.Instance.IDSOLICITUDMED);
                    }

                    //if (cabeceraMed.Accion == "UPDATE") // SI ES UPDATE, ELIMINA EL DETALLE Y VUELVE A INSERTAR
                    //{
                    //    objGenral.Accion = "UPDATE_DESPACHO_HEADER";
                    //    objGenral.Componente = w_numerodocumento;
                    //    objGenral.IdOrdenAtencion = ENTITY_GLOBAL.Instance.IdOrdenAtencion;
                    //    result = SOA_AtencionesSpring.ListaConsultaExternaEmergencia(objGenral);                      
                    //}        
                    if (result > 0)
                    {
                        cabeceraMed.IdSolicitudProducto = result;
                        result = svcSS_FA_SolicitudProducto.setMantenimiento(cabeceraMed, listadetalleMed);
                        List<SS_HCE_ConsultaExterna> listaExterna = new List<SS_HCE_ConsultaExterna>();
                        if (result > 0)
                        {
                            List<SS_FA_SolicitudProductoDetalle> ListarDetalleMaterial = new List<SS_FA_SolicitudProductoDetalle>();
                            SS_FA_SolicitudProducto ojddetallebus = new SS_FA_SolicitudProducto();
                            ojddetallebus.IdSolicitudProducto = result;
                            ojddetallebus.IdEpisodioAtencion = (long)ENTITY_GLOBAL.Instance.EpisodioAtencion;
                            ojddetallebus.IdPaciente = (int)ENTITY_GLOBAL.Instance.PacienteID;
                            ojddetallebus.EpisodioClinico = (int)ENTITY_GLOBAL.Instance.EpisodioClinico;
                            ojddetallebus.UnidadReplicacion = ENTITY_GLOBAL.Instance.UnidadReplicacion.Trim();
                            ojddetallebus.Accion = "LISTAR";
                            ListarDetalleMaterial = svcSS_FA_SolicitudProducto.ListarDetalle(ojddetallebus);
                            ...
                            foreach (SS_FA_SolicitudProductoDetalle objEntity2 in ObjLista2)
                            {
                                ...
                                objGenralDetalle.UsuarioModificacion = String.Concat(ENTITY_GLOBAL.Instance.UnidadReplicacion.Trim(), Convert.ToString(result));
                                objGenralDetalle.Linea = objEntity2.Secuencia;
                                //objGenralDetalle.Componente = objEntity2.CodigoComponente;
                                objGenralDetalle.IndicadorEPS = result;
                                objGenralDetalle.TipoOrdenMedica = Convert.ToInt32(objEntity2.Cantidad);
                                objGenralDetalle.Componente = objEntity2.CodigoComponente;
                                objGenralDetalle.SecuenciaHCE = objEntity2.UsuarioModificacion;
                                // agrgado 20211228 
                                objGenralDetalle.IdOrdenAtencion = ENTITY_GLOBAL.Instance.IdOrdenAtencion;
                                objGenralDetalle.LineaOrdenAtencion = ENTITY_GLOBAL.Instance.LineaOrdenAtencion;
                                objGenralDetalle.IdConsultaExterna = result;

                                if (objEntity2.Accion != "DELETE")
                                {
                                    int idResulta = 0;
                                    var objGenralEnf = new SS_HCE_RecetaConsultaExterna();
                                    objGenralEnf.IdOrdenAtencion = ENTITY_GLOBAL.Instance.IdOrdenAtencion;
                                    objGenralEnf.SecuenciaHCE = objGenralDetalle.SecuenciaHCE;
                                    objGenralEnf.LineaOA = ENTITY_GLOBAL.Instance.LineaOrdenAtencion;
                                    objGenralEnf.IdPaciente = (int)ENTITY_GLOBAL.Instance.PacienteID;
                                    objGenralEnf.Componente = objEntity2.CodigoComponente;
                                    if (objGenralDetalle.SecuenciaHCE == null || objGenralDetalle.SecuenciaHCE == "")
                                    {
                                        foreach (SS_FA_SolicitudProductoDetalle EntyList in ListarDetalleMaterial)
                                        {
                                            if (objEntity2.CodigoComponente == EntyList.CodigoComponente && objEntity2.TipoComponente == "M")
                                            {
                                                objGenralEnf.SecuenciaHCE = EntyList.SecuencialHCE;
                                            }
                                        }
                                    }

                                    objGenralEnf.Unidadmedida = 1;
                                    objGenralEnf.linea = objEntity2.Linea;
                                    objGenralEnf.Familia = objEntity2.Familia;
                                    objGenralEnf.Subfamilia = objEntity2.SubFamilia;
                                    objGenralEnf.Cantidad = Convert.ToString(Convert.ToInt64(objEntity2.Cantidad));
                                    objGenralEnf.Usuario = ENTITY_GLOBAL.Instance.USUARIO;
                                    objGenralEnf.IndicacionEspecifica = Session["OPCIONALMACEN"].ToString();
                                    objGenralEnf.lineamax = result;
                                    if (objEntity2.IdPacienteReferencia != 1)
                                    {
                                        //para materia medico
                                        if (objEntity2.Accion == "DELETE")
                                        {
                                            objGenralEnf.TIPOORDENATENCION = 2500;
                                        }
                                        else
                                        {
                                            objGenralEnf.TIPOORDENATENCION = 2000;
                                        }
                                    }
                                    else
                                    {
                                        //para medicamento
                                        objGenralEnf.TIPOORDENATENCION = 3000;
                                    }

                                    if (objEntity2.Accion == "NUEVO" || objEntity2.Accion == "UPDATE")
                                    {
                                        idResulta = SOA_AtencionesSpring.ListaRecetaConsultaExternaEmergencia(objGenralEnf);
                                    }
                                }
                            }
                        }
                    ...
                    if (result > 0)
                    {
                        ...
                        LocalEnty.EpisodioClinico = (int)SoluccionSalud.Entidades.Entidades.ENTITY_GLOBAL.Instance.EpisodioClinico;
                        LocalEnty.IdEpisodioAtencion = (long)SoluccionSalud.Entidades.Entidades.ENTITY_GLOBAL.Instance.EpisodioAtencion;
                        LocalEnty.IdSolicitudProducto = result;
                        LocalEnty.Accion = "ID_REGISTRAR";
                        object pk = svcSS_FA_SolicitudProducto.Listar(LocalEnty);
                        ENTITY_GLOBAL.Instance.GuiaPedido = pk.ToString();
        ...
        //Agregar nuevo Metodo Save para el nuevo Form de Visor Receta de Añadir Med
        public System.Web.Mvc.ActionResult Save_Solicitud02(String selectionArray1, String selectionArray2)
        {
            int contadorBool = 0;
            int constadorNullos = 0;
            int contadorTotal = 0;
            ...
            if (selectionArray1.Trim().Length > 0)
            {
                try
                {
                    ...
                    {
                        ObjLista2 = (List<SS_FA_SolicitudProductoDetalle>)Newtonsoft.Json.JsonConvert.DeserializeObject(selectionArray2, typeof(List<SS_FA_SolicitudProductoDetalle>));
                        contadorTotal = ObjLista2.Count();
                    }

                    if (ENTITY_GLOBAL.Instance.despachoExtra)
                    {
                        foreach (var itemFiltro in ObjLista2)
                        {
                            if (string.IsNullOrEmpty(itemFiltro.Accion))
                            {
                                constadorNullos = constadorNullos + 1;
                                //return showMensajeNotify("Información", "No existen cambios.", "INFORMACIÓN");
                            }
                            else
                            {
                        ...
                        contadorBool = contadorTotal - constadorNullos;
                        if (constadorNullos == 0)
                        {
                            return showMensajeNotify("Información", "No existen cambios.", "INFORMACIÓN");
                        }
                    }
                    ...
                    var objGenral = new SS_HCE_ConsultaExterna();
                    objGenral.UnidadReplicacion = ENTITY_GLOBAL.Instance.UnidadReplicacion.Trim();
                    objGenral.SecuenciaHCE = String.Concat(ENTITY_GLOBAL.Instance.UnidadReplicacion.Trim(), Convert.ToString(result));
                    objGenral.IndicadorEPS = result;
                    objGenral.UsuarioCreacion = ENTITY_GLOBAL.Instance.USUARIO;
                    objGenral.Componente = ENTITY_GLOBAL.Instance.CodigoOA;
                    objGenral.IdOrdenAtencion = ENTITY_GLOBAL.Instance.IdOrdenAtencion;
                    objGenral.Linea = ENTITY_GLOBAL.Instance.LineaOrdenAtencion;
        ...
        {
            Log.Information("VisorRecetaController - Delete_SolicitudDetalle");
                ...
                foreach (SS_FA_SolicitudProductoDetalle objEntity1 in listaSave)
                {
                        ...
                        objEntity1.CodigoComponente == codigo)
                    {
                        if ((objEntity1.Accion + "").Trim() != "")
                        {
                            ...
                            objDelete.EpisodioClinico = (int)ENTITY_GLOBAL.Instance.EpisodioClinico;
                            objDelete.IdEpisodioAtencion = (long)ENTITY_GLOBAL.Instance.EpisodioAtencion;
                            objDelete.SecuencialHCE = objEntity1.SecuencialHCE;
                            ...
                            objDelete.TipoComponente = objEntity1.TipoComponente;
                            objDelete.Accion = "DELETE";
        ...
        {
            Log.Information("VisorRecetaController - AddMedicamentoDevol");
            ...
            return crearWindowRegistro("DevolucionMed/AddMedicamentoDevol", objModel, "");
            //return View("UsuarioRegistro", LocalEnty);
        ...
        {
            Log.Information("VisorRecetaController - Index2");
            setVariablesTempSession_Default(UTILES_MENSAJES.PATH_MAIN_HCLINICA);
            ////OBTENER DATOS DE LA PERSONA-MEDICO-USUARIO
            List<VW_PERSONAPACIENTE> lista = new List<VW_PERSONAPACIENTE>();
            VW_PERSONAPACIENTE obj = new VW_PERSONAPACIENTE();
            ...
            return View("DevolucionMed/Index2", obj);
            //  return View(obj);
        }

        public PartialViewResult PanelWest9(string containerId)
        {
            Log.Information("VisorRecetaController - PanelWest9");
            return new PartialViewResult
            {
                RenderMode = RenderMode.AddTo,
                ContainerId = containerId,
                Model = ENTITY_GLOBAL.Instance,
                ViewName = "DevolucionMed/PanelWest9",
                WrapByScriptTag = false // we load the view via Loader with Script mode therefore script tags is not required
            };
        }
        public PartialViewResult PanelEast9(string containerId)
        {
            Log.Information("VisorRecetaController - PanelEast9");
            return new PartialViewResult
            {
                Model = ENTITY_GLOBAL.Instance,
                RenderMode = RenderMode.AddTo,
        ...
        {
            Log.Information("VisorRecetaController - PanelNorth9");
            ////////////////////////
            VW_ATENCIONPACIENTE_GENERAL vistaGenSelec = new VW_ATENCIONPACIENTE_GENERAL();
            if (Session["VW_ATENCIONPACIENTE_GEN_SELECC"] != null)
            {
        ...
        public System.Web.Mvc.ActionResult AddMedicamentoVis(String MODO)
        {
            Log.Information("VisorRecetaController - AddMedicamentoVis");
            var objModel = new SS_FA_SolicitudProductoDetalle();
            objModel.Accion = MODO;
            objModel.UnidadReplicacion = ENTITY_GLOBAL.Instance.UnidadReplicacion;
            objModel.IdPaciente = (int)ENTITY_GLOBAL.Instance.PacienteID;
            objModel.EpisodioClinico = (int)ENTITY_GLOBAL.Instance.EpisodioClinico;
            objModel.IdEpisodioAtencion = Convert.ToInt32(ENTITY_GLOBAL.Instance.EpisodioAtencion);
            //aqui me quede
            return crearWindowRegistro("AddMedicamentoVis", objModel, "");
            //return View("UsuarioRegistro", LocalEnty);

        }

        public System.Web.Mvc.ActionResult eventoAgregarHistoricoFormulario9(string formato, string accion, string idWindow,
           Nullable<long> idEpiAtencionCopy, Nullable<int> idEpiClinicoCopy, string UnidadReplicacionCopy, String data)
        {
            Log.Information("VisorRecetaController - eventoAgregarHistoricoFormulario9");
            List<SS_FA_SolicitudProductoDetalle> dataSave = null;
            var varPasarDetalle = new SS_FA_SolicitudProductoDetalle();

            List<SS_FA_SolicitudProductoDetalle> ObjListar = (List<SS_FA_SolicitudProductoDetalle>)Newtonsoft.Json.JsonConvert.DeserializeObject(data, typeof(List<SS_FA_SolicitudProductoDetalle>));
            ...
            try
            {
                if (ObjListar != null)
                {
                    if (ObjListar.Count > 0)
                    {
                        foreach (SS_FA_SolicitudProductoDetalle objadd in ObjListar)
                        {
                            ...
                            varPasarDetalle.Version = objadd.Version;
                            varPasarDetalle.Cantidad = objadd.Cantidad;
                            varPasarDetalle.Estado = objadd.Estado;
                            varPasarDetalle.Accion = objadd.Accion;
                            varPasarDetalle.UsuarioCreacion = objadd.UsuarioCreacion;
                            varPasarDetalle.indicaciones = objadd.indicaciones;
                            varPasarDetalle.UnidadReplicacionReferencia = objadd.UnidadReplicacionReferencia;
                            var txtComp = X.GetCmp<TextField>("RecepcionaRecurso");
                            txtComp.SetValue(JSON.Serialize(varPasarDetalle));
